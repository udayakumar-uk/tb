...
if(!empty($_POST['delet']))
{
	...
	for($i=1;$i<=$_POST['n'];$i++)
	{
		...
		{
			if($ids=="")
			$ids=stripslashes(mysqli_real_escape_string($_POST['delt'.$i]));
			else
			$ids.="','".stripslashes(mysqli_real_escape_string($_POST['delt'.$i]));
		}
	}

	$seldata=executework("select * from tob_auction where tdate in ('".$ids."') and platf in(select id from tob_platform where state='".mysqli_real_escape_string($stat)."')");
	$cnt=@mysqli_num_rows($seldata);
	
	if($cnt>0)
	{
		$deldata=executework("delete from tob_auction where tdate in ('".$ids."') and platf in(select id from tob_platform where state='".mysqli_real_escape_string($stat)."')");
		
	}
	$delgr=executework("delete from tob_grade where state='".mysqli_real_escape_string($stat)."' and pdate in ('".$ids."')");
