...
if(isset($_SESSION['tobadmin']) && !empty($_SESSION['tobadmin']))
{
	while($rowmn=@mysqli_fetch_array($selm))
	{
		...
		//echo ("select * from tob_pages where menu_id='".$rowmn['id']."' and isactive=1 order by morder");
		$cnt2=@mysqli_num_rows($sel2);
		if($cnt2>0)
		$cls="class='caret'";
		else
...
<ul id="nav">
	...
	if($_SESSION['tobadmin']=='admin')
	{
	 if($cnt2>0) {
	 ?>
    <li style="padding-left:-50px"><a href="<?php echo $link; ?>"><?php echo $rowmn['page']; ?></a>
    <ul>
      <?php while($row=@mysqli_fetch_array($sel2)) { 
		
			//print_r($row);
			$sel3=executework("select * from tob_pages where menu_id='".$row['id']."' and isactive=1 order by morder");
		//	echo ("select * from tob_pages where menu_id='".$row['id']."' and isactive=1 order by morder");
			$cnt3=@mysqli_num_rows($sel3);
			if($cnt3>0)
