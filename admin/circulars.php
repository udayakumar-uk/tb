...
<?php
	...
	if(!empty($_GET['edit'])) {
		...
		$tselect=executework("select * from tob_circulars where title='". $_POST['title']."' and stype='".$_POST['stype']."'");
...
<section id="adminLayout">
	...
	<h2 class="admin-title">Post Circulars </h2>

	<div id="adminTab">
	<nav>
		<div class="nav nav-tabs align-items-center" id="nav-tab" role="tablist">
			<button class="nav-link <?php if(empty($_GET['page_index'])){ ?> active <?php } ?>" id="post-tab" data-bs-toggle="tab" data-bs-target="#post-circular" type="button" role="tab" aria-controls="nav-post" aria-selected="true">Post Circular</button>
			<button class="nav-link <?php if(!empty($_GET['page_index']) && $_GET['page_index']!=""){ ?> active <?php } ?>" id="view-tab" data-bs-toggle="tab" data-bs-target="#view-circular" type="button" role="tab" aria-controls="nav-view" aria-selected="false">View Circular</button>
			...
			<?php if(!empty($_GET['exist']) && $_GET['exist']==1){ ?>
				<div class="alert alert-danger d-flex align-items-center py-1 px-2 m-0 ms-auto" role="alert">
					...
					<span> Given Circular Details Alredy Exist </span>
				</div>
			...
			<?php } else if((!empty($_GET['succ']) && $_GET['succ']==1) || (!empty($_GET['succ']) && $_GET['succ']==2) || (!empty($_GET['succ']) && $_GET['succ']==3) || (!empty($_GET['succ']) && $_GET['succ']==4)){ ?>
				<div class="alert alert-success d-flex align-items-center py-1 px-2 m-0 ms-auto" role="alert">
					...
					<?php if(!empty($_GET['succ']) && $_GET['succ']==1){ ?>
						<span> New Circular Posted Successfully </span>
					<?php } else if(!empty($_GET['succ']) && $_GET['succ']==2){ ?>
						<span> Selected Circular Successfully Move To Archives </span>
					<?php } else if(!empty($_GET['succ']) && $_GET['succ']==3){ ?>
						<span> Selected Circular Details Modify Successfully </span>
					<?php } else if(!empty($_GET['succ']) && $_GET['succ']==4){ ?>
						<span> Selected Circular Details Deleted Successfully </span>
					<?php } ?>
				</div>
			<?php } ?>
		</div>
	...
	<div class="tab-content" id="nav-tabContent">
		<div class="tab-pane fade <?php if(empty($_GET['page_index'])){ ?> show active <?php } ?>" id="post-circular" role="tabpanel" aria-labelledby="post-circular-tab" tabindex="0">
	...
	<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1" onsubmit="return check(this);">
		...
		<div class="form-group">
			<label for="stype" class="form-label">Circular Type</label>
		...
		<div class="row">
			...
			<div class="form-group col-md-6">
				...
				<input name="title" class="form-control" type="text" id="title" size="40" value="<?php echo $trow['title']?>" />
			</div>
			
			<div class="form-group col-md-6">
				<label for="htitle" class="form-label">Title (Hindi)</label>
				<input name="htitle" class="form-control" type="text" id="htitle" size="40" value="<?php echo $trow['htitle']?>" />
			</div>
		</div>

		<div class="form-group">
			<label for="description" class="form-label">Description of Circular</label>
		...
		<div class="form-group">
			<label for="hdescription" class="form-label">Description of Circular (Hindi)</label>
		...
		<?php if(!empty($trow['tfile'])){?>
			<a href="../tbdata/circularfiles/<?php echo $trow['tfile']?>" target="_blank">
				<img src="../tbdata/circularfiles/<?php echo $trow['tfile']?>" alt="document" height="100" width="100" class="border rounded-3"/>
			</a>
		<?php }?>
	...
	<div class="tab-pane fade <?php if(!empty($_GET['page_index']) && $_GET['page_index']!=""){ ?> show active <?php } ?>" id="view-circular" role="tabpanel" aria-labelledby="view-statics-tab" tabindex="1">
	...
	<div class="d-flex align-items-center justify-content-between my-3">
		...
		if ($count > 0) {
			...
			<ul class="pagination">
				<?php for($im=1;$im<=$pages;$im++) {
					if($page12 != $im){ ?>
						<li class="page-item"><a class="page-link hlink1" href="circulars.php?page_index=<?php echo "$im" ?>"><?php echo "$im" ?></a></li>
					<?php } else{ ?>
		...
		<div class="table-responsive">
			<table class="table table-bordered ">
                ...
                <tr>
					...
					<td><a href="<?php echo $link;?>" target="<?php if($link!="#"){?>_blank<?php }?>" class="b"><?php echo $row['stype']?></a></td>
					<td><a href="<?php echo $link;?>" target="<?php if($link!="#"){?>_blank<?php }?>" class="b"><?php echo $row['title']?></a></td>
		...
		<?php if ($pages > 1) { ?>
			<div class="text-end">
				<?php if($page_index != 1){
					$pre=$page_index-1; ?>
					<button name="button" type="button" class="btn btn-sm btn-primary fbutton" onclick="location.href='circulars.php?page_index=<?php echo $pre ?>'" />Previous</button>
				<?php
				}
				if($page_index < $pages) {
				$next=$page_index+1; ?>
					<button name="button" type="button" class="btn btn-sm btn-primary fbutton" onclick="location.href='circulars.php?page_index=<?php echo $next ?>'" />Next  </button>
				<?php } ?>
			</div>
		<?php } ?>
