...
if(!empty($_FILES['image']['name']))
{
	...
	//echo "select * from master_employee where Employee_number='".$_GET['eid']."'";
	$count=@mysqli_num_rows($sel);
	if($count>0)
	{
		$rowttt=@mysqli_fetch_array($sel);
		$target_pathn = "employeeimage/";
		$f=basename($_FILES['image']['name']);
		$len=strlen($f);
		$pos=strpos($f,'.');
		$ext=substr($f,$pos,$len);
		$f1=$rowttt['Employee_number'].$ext;
		...
		$update=executework("update master_employee set image='".$f1."' where Employee_number='".$rowttt['Employee_number']."'");
		redirect("employee.php?eid=".$rowttt['Employee_number']);
	}
	else
	{
...
function GetDays($sStartDate, $sEndDate){  
  ...
  return $aDays;  
	...
	if(!empty($rowmd['E_whether DR/PR']))
	{
		if($rowmd['E_whether DR/PR']=='PR')
		$rowmd['E_whether DR/PR']='Promotion';
		else
		$rowmd['E_whether DR/PR']='Direct Recruitment';
	}
	if(!empty($rowmd['E_Regular/Temp/Adhoc']))
	{
		if($rowmd['E_Regular/Temp/Adhoc']=='R')
		$rowmd['E_Regular/Temp/Adhoc']='Regular';
		else if($rowmd['E_Regular/Temp/Adhoc']=='T')
		$rowmd['E_Regular/Temp/Adhoc']='Temporary';
	}
	if(!empty($rowmd['E_TECH/NON TECH']))
	{
		if($rowmd['E_TECH/NON TECH']=='T')
		$rowmd['E_TECH/NON TECH']='Technical';
		else if($rowmd['E_TECH/NON TECH']=='NT')
		$rowmd['E_TECH/NON TECH']='Non-Technical';
	}
...
//	$row=@mysqli_fetch_array($select);
	...
	if($rowmd['image']!="")
	$img="employeeimage/".$rowmd['image'];
	else
	$img="noimage.png";
	
	$str=executework("select * from tob_designation where id='".$rowmd['Designation']."'");
	//echo "select * from tob_designation where id='".$rowmd['Designation']."'";
	$rtr=@mysqli_fetch_array($str);
	
	$selectg=executework("select * from tob_platfrm where id='".$rowmd['place_working']."'");
	...
	if(!empty($_GET['eid']))
	{
	    $qr=" and Employee_number='".$_GET['eid']."'";
	    $m=0;
	}
	if($m<0)
	$m=0;

	$selectp=executework("select * from master_employee where Employee_number<>''".$qr." order by Employee_number limit ".$m.",1");
	//$selectp=executework("select * from master_employee where Employee_number='".$_GET['eid']."' order by Employee_number");
	$countp=@mysqli_num_rows($selectp);
	$rowp=@mysqli_fetch_array($selectp);
	
	$selectn=executework("select * from master_employee where Employee_number<>'' order by Employee_number limit ".$n.",1");
	$countn=@mysqli_num_rows($selectn);
	$rown=@mysqli_fetch_array($selectn);
?>
<a name="top" id="top"></a>
<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td colspan="2" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabbor">
      <tr>
        <td width="90%" height="25" bgcolor="#F7F7F7"><table width="100%" border="0" cellpadding="4" cellspacing="1">
  <tr>
    <td width="74%" class="style1">&nbsp;</td>
    <td width="8%" class="style8"><?php if($rowp['Employee_number']!="") { ?><a href="employee.php?eid=<?php echo $rowp['Employee_number'] ?>&n=<?php echo $n-1 ?>">&lt;&lt; Previous</a><?php } ?></td>
    <td width="18%" class="style8"><?php if($rown['Employee_number']!="") { ?><a href="employee.php?eid=<?php echo $rown['Employee_number'] ?>&n=<?php echo $n+1 ?>">Next &gt;&gt;</a><?php } ?></td>
  </tr>
</table></td>
        </tr>
        <?php
		...
		?>
      <tr>
        <td height="25" bgcolor="#F7F7F7"><div style="margin-right:100px;" align="right" class="style8a"><a href="javascript:void(0)" class="b" onclick="MM_openBrWindow('printemployee.php?eid=<?php echo $rowmd['Employee_number'] ?>','','width=600,height=1000')"><strong>Print</strong></a></div></td>
      </tr>
      <?php
	  	}
	  ?>
...
<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1" onsubmit="return validate_image();" >
        ...
        <ul id="tabs">
      <li><a href="#" name="#tab1">General Profile</a></li>
      <li><a href="#" name="#tab2">Personal Data</a></li>
  </ul>
                    <div id="content">
      <div id="tab1">
        ...
        <table width="800" border="0" align="center" cellpadding="2" cellspacing="2" bgcolor="#FFFFFF">
                      <tr>
                        <td colspan="3" bordercolor="#000000" bgcolor="#FFFFFF"><table width="800" border="1" align="center" cellpadding="4" cellspacing="1" bordercolor="#FFFFFF" bgcolor="#000000">
                          <tr bgcolor="#F5F5F5">
                            <td width="24%" bgcolor="#F5F5F5" class="8a">Name</td>
                            <td width="20%" bgcolor="#F5F5F5" class="8"><?php echo $rowmd['Name'];?></td>
                            <td class="style8a">Employee Number</td>
                            <td class="style8"><?php echo $rowmd['Employee_number']?></td>
                          ...
                          <tr bgcolor="#DDDDDD">
                            ...
                            <td bgcolor="#DDDDDD" class="style8"><?php echo $rtr['designation']; ?></td>
                            <td bgcolor="#DDDDDD" class="style8a">Group</td>
                            <td bgcolor="#DDDDDD" class="style8"><?php echo $rowmd['E_CLASSIFICATION']; ?></td>
                          </tr>
                          <tr bgcolor="#F5F5F5">
                            ...
                            <td bgcolor="#F5F5F5" class="style8"><?php echo $rowmd['E_SECTION']; ?></td>
                          </tr>
                          <tr bgcolor="#DDDDDD">
                            <td bgcolor="#DDDDDD" class="style8a">Working Since</td>
                            <td bgcolor="#DDDDDD" class="style8"><?php echo datepattrn($rowmd['Working_from']); ?></td>
                            <td>&nbsp;</td>
                            <td>&nbsp;</td>
                          </tr>
                          <tr bgcolor="#DDDDDD">
                            <td bgcolor="#F5F5F5" class="style8a">Pay Band</td>
                            <td bgcolor="#F5F5F5" class="style8"><?php echo $rowmd['E_Pay_Band']; ?></td>
                            <td width="16%" bgcolor="#F5F5F5">&nbsp;</td>
                            <td width="18%" bgcolor="#F5F5F5">&nbsp;</td>
                            </tr>
                          <tr bgcolor="#DDDDDD">
                            <td class="style8a">Grade Pay</td>
                            <td class="style8"><?php echo $rowmd['E_Grade_Pay']; ?></td>
                            <td class="style8a">Date Of Retirement</td>
                            <td class="style8"><?php echo datepattrn($rowmd['E_DOR']); if($rowmd['status']==1) { ?>(<?php echo $rowmd['emp_status']; ?>)<?php } ?>
							</td>
                            </tr>
                   ...
                   <?php
				   $selser=executework("select * from service_details where E_NO='".$rowmd['Employee_number']."' order by E_FROM");
		...
		$stv=executework("select * from tob_designation where id='".$rows['E_CADRE']."'");
		$rov=@mysqli_fetch_array($stv);
  ...
  $rowtg1=@mysqli_fetch_array($selectg1);
						?>
                          <tr bgcolor="<?php echo $bg ?>">
                            <td><?php echo $s; ?></td>
                            <td><?php echo $rov['designation']; ?></td>
                  ...
                  <?php
				   ...
				   $selv=executework("select * from disciplinary_det where E_NO='".$rowmd['Employee_number']."' order by id");
				   //echo "select * from disciplinary_det where E_NO='".$rowmd['Employee_number']."' order by id";
...
<table width="800" border="0" align="center" cellpadding="2" cellspacing="2" bgcolor="#FFFFFF">
                      <tr>
                        <td colspan="3" bgcolor="#FFFFFF"><table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#FFFFFF" bgcolor="#000000">
                          <tr bgcolor="#F5F5F5">
                            ...
                            <td width="29%" class="style8"><?php echo datepattrn($rowmd['E_DOB']); ?></td>
                            <td width="23%" bgcolor="#F5F5F5" class="style8a">Category</td>
                            <td width="26%" bgcolor="#F5F5F5" class="style8"><?php echo $rowmd['E_CATEGORY']; ?></td>
                          </tr>
                          <tr bgcolor="#F5F5F5">
                            <td bgcolor="#DDDDDD" class="style8a">Date Of Entry Into Service</td>
                            <td bgcolor="#DDDDDD" class="style8"><?php echo datepattrn($rowmd['E_DOJ']); ?></td>
                            <td bgcolor="#DDDDDD" class="style8a">DR/PR</td>
                            <td bgcolor="#DDDDDD" class="style8"><?php echo $rowmd['E_whetherDRPR']; ?></td>
                          </tr>
                          <tr bgcolor="#F5F5F5">
                            <td class="style8a">Designation At Joining Service</td>
                            <td class="style8"><?php echo $rowmd['E_JOINED_AS']; ?></td>
                            <td bgcolor="#F5F5F5" class="style8a">Regular/Temporary/Adhoc</td>
                            <td bgcolor="#F5F5F5" class="style8"><?php echo $rowmd['E_RegularTempAdhoc']; ?></td>
                          </tr>
                          <tr bgcolor="#F5F5F5">
                            ...
                            <td bgcolor="#DDDDDD" class="style8"><?php echo $rowmd['E_NATIVE']; ?></td>
                          </tr>
                          <tr bgcolor="#F5F5F5">
                            <td class="style8a">Educational Qualifications</td>
                            <td class="style8"><?php echo $rowmd['E_QUA']; ?></td>
                            <td class="style8a">Present Residing Address</td>
                            <td class="style8"><?php echo $rowmd['E_ADDRESS']; ?></td>
                          </tr>
                          <tr bgcolor="#F5F5F5">
                            <td bgcolor="#DDDDDD" class="style8a">Technical/Non-Technical</td>
                            <td bgcolor="#DDDDDD" class="style8"><?php echo $rowmd['E_TECHNON_TECH']; ?></td>
                            <td bgcolor="#DDDDDD" class="style8a">Phone Nos.</td>
                            <td bgcolor="#DDDDDD" class="style8"><?php echo $rowmd['E_Phone1']; ?>,<?php echo $rowmd['E_Phone2']; ?></td>
                          </tr>
                          <tr bgcolor="#F5F5F5">
                            <td bgcolor="#F5F5F5" class="style8a">Physical Handicapped</td>
                            <td bgcolor="#F5F5F5" class="style8"><?php echo $rowmd['E_Handicapped']; ?></td>
                   ...
                   <?php
				   $selpr=executework("select * from promotion_det where E_NO='".$rowmd['Employee_number']."'");
			...
			$std=executework("select * from tob_designation where id='".$rowp[$des]."'");	
			$roty=@mysqli_fetch_array($std);			
						?>
                          <tr bgcolor="<?php echo $bg ?>">
                            <td><?php echo $s ?></td>
                            <td><?php echo $roty['designation']; ?></td>
				 ...
				 ?>
                   <?php
				   $selpr=executework("select * from tob_aaleaves where empno='".$rowmd['Employee_number']."' order by edate");
				   $cntp=@mysqli_num_rows($selpr);
				   if($cntp>0)
				   {
				   		//$rowp=@mysqli_fetch_array($selpr);
				 ...
				 ?>
                      <tr bgcolor="#FFFFFF">
                        <td colspan="3" class="style8"><div align="center"><input type="button" name="button" id="button" value=" Back " onclick="javascript:history.back();" /></div></td>
                      </tr>
                      <tr bgcolor="#FFFFFF">
                        <td colspan="3" class="style8">&nbsp;</td>
                      </tr>
  ...
  <p>&nbsp;</p>
          ...
          <div align="center"><input type="button" name="button2" id="button2" value=" Back " onclick="location.href='employeedata.php'" /></div> </div></td>
      </tr>
    </table></td>
  </tr>
  <?php
