...
if(!empty($_FILES['image']['name']))
{
	...
	if($count>0)
	{
		...
		$update=executework("update master_employee set image='".$f1."' where Employee_number='".$rowmd['Employee_number']."'");
		redirect("employeedata.php?mid=1");
	}
	else
	{
...
if(!empty($_POST['subm']))
{
	...
	$_POST['Designation']=0;
	if(!empty($_POST['status']) && $_POST['status']=='In-Service')
	$checks=0;
	else
	$checks=1;
	//'".date('Y-m-d',strtotime($_POST['Working_from']))."',
	 $work_from = $_POST['Working_from'] ?? "";
	  $ee_dob = $_POST['E_DOB'] ?? "";
	  $ee_dor = $_POST['E_DOR'] ?? "";
	$upd=executework("update master_employee set Name='".$_POST['Name']."',Designation='".$_POST['Designation']."',E_CLASSIFICATION='".$_POST['E_CLASSIFICATION']."',place_working='".$_POST['place_working']."',E_NATIVE='".$_POST['E_NATIVE']."',E_SECTION='".$_POST['E_SECTION']."',E_ADDRESS='".$_POST['E_ADDRESS']."',Working_from='".$work_from."',E_Phone1='".$_POST['E_Phone1']."',E_Phone2='".$_POST['E_Phone2']."',E_Pay_Band='".$_POST['E_Pay_Band']."',E_QUA='".$_POST['E_QUA']."',E_Grade_Pay='".$_POST['E_Grade_Pay']."',E_TECHNON_TECH='".$_POST['E_TECH']."',E_DOB='".$ee_dob ."',E_CATEGORY='".$_POST['E_CATEGORY']."',E_DOJ='".$_POST['E_DOJ']."',E_whetherDRPR='".$_POST['drpr']."',E_JOINED_AS='".$_POST['E_JOINED_AS']."',E_RegularTempAdhoc='".$_POST['E_Regular']."',E_DOR='".$ee_dor."',E_Handicapped='".$_POST['E_Handicapped']."',emp_status='".$_POST['status']."',status='".$checks."' where  Employee_number='".$eid."'");
	...
	{
		//echo "ss=".$_POST['sid'.$i];
			  $ee_from = $_POST['E_FROM'.$i] ?? "";
			  $ee_to = $_POST['E_TO'.$i] ?? "";
		if(!empty($_POST['sid'.$i]))
		{
			if(empty($_POST['E_CADRE'.$i]))
			$_POST['E_CADRE'.$i]=0; 
			/*$ee_from = date('Y-m-d',strtotime($_POST['E_FROM'.$i])) ?? "";
			$ee_to = date('Y-m-d',strtotime($_POST['E_TO'.$i])) ?? "";*/
			$qr='';
			if($ee_from!='')
			$qr.=",E_FROM='".$ee_from."'";
			if($ee_to!='')
			...
			$qr.=",E_TO=null";
			echo "update service_details set E_PLACE='".$_POST['E_PLACE'.$i]."',E_PF='".$_POST['E_PF'.$i]."',E_CADRE='".$_POST['E_CADRE'.$i]."'".$qr." where id=".$_POST['sid'.$i];
			$upd=executework("update service_details set E_PLACE='".$_POST['E_PLACE'.$i]."',E_PF='".$_POST['E_PF'.$i]."',E_CADRE='".$_POST['E_CADRE'.$i]."'".$qr." where id=".$_POST['sid'.$i]);
		}
		else
		{
			if(!empty($_POST['E_CADRE'.$i]))
			{ 
				...
				echo "insert into service_details (E_NO,E_PLACE,E_PF,E_CADRE".$qr.") values('".$eid."','".$_POST['E_PLACE'.$i]."','".$_POST['E_PF'.$i]."','".$_POST['E_CADRE'.$i]."'".$qr1.")";
				$int=executework("insert into service_details (E_NO,E_PLACE,E_PF,E_CADRE".$qr.") values('".$eid."','".$_POST['E_PLACE'.$i]."','".$_POST['E_PF'.$i]."','".$_POST['E_CADRE'.$i]."'".$qr1.")");
				//$int=executework("insert into service_details values('".$eid."','".$_POST['E_PLACE'.$i]."','".$_POST['E_PF'.$i]."','".$_POST['E_CADRE'.$i]."','".$ee_from."','".$ee_to."','0','0','',".$maxid.")");
				$maxid++;
			}
		}
	}
	...
	{
		if(!empty($_POST['pid'.$i]))
		{
			if(empty($_POST['E_DES'.$i]))
			$_POST['E_DES'.$i]=0;
			 $ee_dop= $_POST['E_DOP_DT'.$i] ?? "";
			//date('Y-m-d',strtotime($_POST['E_DOP_DT'.$i]))
			//echo "update promotion_det set E_DES='".$_POST['E_DES'.$i]."',E_DOP_DT='".$ee_dop."',ptype='".$_POST['ptype'.$i]."',orderno='".$_POST['order'.$i]."' where id=".$_POST['pid'.$i];
			$upd=executework("update promotion_det set E_DES='".addslashes($_POST['E_DES'.$i])."',E_DOP_DT='".$ee_dop."',ptype='".addslashes($_POST['ptype'.$i])."',orderno='".addslashes($_POST['order'.$i])."' where id=".$_POST['pid'.$i]);
		}
		else
		{
			if(!empty($_POST['E_DES'.$i]))
			{
			$ee_dop =  $_POST['E_DOP_DT'.$i] ?? "";
				$int=executework("insert into promotion_det values('".$maxid1."','".$eid."','".addslashes($_POST['E_DES'.$i])."','".$ee_dop."','".addslashes($_POST['ptype'.$i])."','".addslashes($_POST['order'.$i])."')");
				$maxid1++;
			}
		}
	}
	for($i=1;$i<=$_POST['vn'];$i++)
	{
		...
		if(!empty($_POST['vid'.$i]))
		{
			$upd=executework("update disciplinary_det set refno='".addslashes($_POST['refno'.$i])."',det='".addslashes($_POST['det'.$i])."',status='".$_POST['status'.$i]."' where id=".$_POST['vid'.$i]);
		}
		else
		{
			if($_POST['refno'.$i]!="")
			{
				$int=executework("insert into disciplinary_det values('".$maxid2."','".$eid."','".addslashes($_POST['refno'.$i])."','".addslashes($_POST['det'.$i])."','".$_POST['status'.$i]."')");
				$maxid2++;
			}
		}
	}
	...
	{
		if(empty($_POST['eob'.$i]))
		$_POST['eob'.$i]=0;
		if(empty($_POST['eaccured'.$i]))
		$_POST['eaccured'.$i]=0;
		if(empty($_POST['eused'.$i]))
		$_POST['eused'.$i]=0;

		if(empty($_POST['hob'.$i]))
		$_POST['hob'.$i]=0;
		if(empty($_POST['haccured'.$i]))
		$_POST['haccured'.$i]=0;
		if(empty($_POST['hused'.$i]))
		$_POST['hused'.$i]=0;
		...
		if(!empty($_POST['lid'.$i]))
		{
		$eDate =  $_POST['edate'.$i] ?? "";
	   // $eDate = 	date('Y-m-d',strtotime($_POST['edate'.$i])) ?? "";
		//echo $_POST['edate'.$i].$i."ooooooooooook";

 // echo "update tob_aaleaves set edate='".$_POST['edate'.$i]."',eob='".$_POST['eob'.$i]."',eaccured='".$_POST['eaccured'.$i]."',etotal=".$etot.",eused=".$_POST['eused'.$i].",ebal=".$ebal.",hob='".$_POST['hob'.$i]."',haccured='".$_POST['haccured'.$i]."',htotal=".$htot.",hused=".$_POST['hused'.$i].",hbal=".$hbal." where id=".$_POST['lid'.$i]."";
 
				$upd=executework("update tob_aaleaves set edate='". $eDate."',eob='".$_POST['eob'.$i]."',eaccured='".$_POST['eaccured'.$i]."',etotal=".$etot.",eused=".$_POST['eused'.$i].",ebal=".$ebal.",hob='".$_POST['hob'.$i]."',haccured='".$_POST['haccured'.$i]."',htotal=".$htot.",hused=".$_POST['hused'.$i].",hbal=".$hbal." where id=".$_POST['lid'.$i]);

//			$sel=executework("select * from tob_aaleaves where empno='".$eid."' and edate='".date('Y-m-d',strtotime($_POST['edate'.$i]))."'");
...
//			}
		...
		else
		{
			...
			{ 
			//date('Y-m-d',strtotime($_POST['edate'.$i]))
			$eDate = $_POST['edate'.$i] ?? "";
				$int=executework("insert into tob_aaleaves values('".$maxid3."','".$eid."','".$eDate."','".$_POST['eob'.$i]."','".$_POST['eaccured'.$i]."',".$etot.",".$_POST['eused'.$i].",".$ebal.",'".$_POST['hob'.$i]."','".$_POST['haccured'.$i]."',".$htot.",".$_POST['hused'.$i].",".$hbal.")");
				$maxid3++;
			}
		}
	}
...
$seldesg=executework("select * from tob_designation");
...
/*$selloc=executework("select * from master_locations order by E_STATION");
$cntl=@mysqli_num_rows($selloc);
$loc=array();
$i=0;
while($rowl=@mysqli_fetch_array($selloc))
{
	$loc[$i]=$rowl['E_STATION'];
	$i++;
}	
*/
...
if(!empty($_POST['Name']))
{
	$name=$_POST['Name'];$designation=$_POST['Designation'];$e_classification=$_POST['E_CLASSIFICATION'];$place_working=$_POST['place_working'];$e_native=$_POST['E_NATIVE'];$e_section=$_POST['E_SECTION'];$e_address=$_POST['E_ADDRESS'];$working_from=$_POST['Working_from'] ?? "";$e_phone1=$_POST['E_Phone1'];$e_phone2=$_POST['E_Phone2'];$e_pay_band=$_POST['E_Pay_Band'];$e_qua=$_POST['E_QUA'];$e_grade_pay=$_POST['E_Grade_Pay'];$e_tech=$_POST['E_TECH'];$e_dob=$_POST['E_DOB'] ?? "";$e_category=$_POST['E_CATEGORY'];$e_doj=$_POST['E_DOJ'];$drpr=$_POST['drpr'];$e_joined_as=$_POST['E_JOINED_AS'];$e_regular=$_POST['E_Regular'];$e_dor=$_POST['E_DOR'] ?? "";$e_handicapped=$_POST['E_Handicapped'];$emp_status=$_POST['status'];$status=$checks;
}
else
{
	//$working_from=datepattrn($row['Working_from']);
	$name=$row['Name'];$designation=$row['Designation'];$e_classification=$row['E_CLASSIFICATION'];$place_working=$row['place_working'];$e_native=$row['E_NATIVE'];$e_section=$row['E_SECTION'];$e_address=$row['E_ADDRESS'];$working_from=$row['Working_from'] ?? "";$e_phone1=$row['E_Phone1'];$e_phone2=$row['E_Phone2'];$e_pay_band=$row['E_Pay_Band'];$e_qua=$row['E_QUA'];$e_grade_pay=$row['E_Grade_Pay'];$e_tech=$row['E_TECHNON_TECH'];$e_dob=$row['E_DOB'] ?? "";$e_category=$row['E_CATEGORY'];$e_doj=$row['E_DOJ'];$drpr=$row['E_whetherDRPR'];$e_joined_as=$row['E_JOINED_AS'];$e_regular=$row['E_RegularTempAdhoc'];$e_dor=$row['E_DOR'] ?? "";$e_handicapped=$row['E_Handicapped'];$emp_status=$row['emp_status'];$status=$row['status'];
}
?>
...
<form action="memployee.php" method="post" enctype="multipart/form-data" name="form1" id="form1" >
                    <br />
                      ...
                      <tr>
                        <td colspan="3" bordercolor="#000000" bgcolor="#FFFFFF"><table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#FFFFFF" bgcolor="#000000">
                          <tr bgcolor="#F5F5F5">
                            ...
                            <td width="34%" bgcolor="#F5F5F5" class="style8"><?php echo $rowmd['Employee_number']; ?>
                              <label></label></td>
                          ...
                          <tr bgcolor="#F5F5F5">
                            ...
                            <td class="style8"><label>
                              <select name="Designation" id="Designation">
								...
								{
								?>
                                <option value="<?php echo $desgi[$i] ?>"><?php echo $desg[$i] ?></option>
                                <?php
								}
								?>
                              </select>
...
?>
                            <tr bgcolor="#F5F5F5">
                              ...
                              <script>sel_combo("status",'<?php echo $emp_status; ?>');</script>
                              <td class="style8a">Total Service Completed</td>
                              <td class="style8"><?php if($tarr!='invalid') echo $tarr[0]. " Years ".$tarr[1]." Months ".$tarr[2]." Days"; ?>
                                  <label>
				   ...
				   ?>
                      ...
                      <tr>
                        <td colspan="3" bgcolor="#FFFFFF" class="style8"><table width="100%" border="1" align="center" cellpadding="4" cellspacing="1" bordercolor="#FFFFFF" bgcolor="#000000">
                          <tr>
                            <td width="9%" rowspan="2" bgcolor="#DDDDDD" class="style8a"><div align="center">S.No</div></td>
                            <td width="19%" rowspan="2" bgcolor="#DDDDDD" class="style8a"><div align="center">Designation</div></td>
                            <td width="26%" rowspan="2" bgcolor="#DDDDDD" class="style8a"><div align="center">Place Of Working</div></td>
                            <td width="18%" rowspan="2" bgcolor="#DDDDDD" class="style8a"><div align="center">Place Code</div></td>
                            <td colspan="2" bgcolor="#DDDDDD" class="style8a"><div align="center">Duration</div></td>
                            <td width="12%" rowspan="2" bgcolor="#DDDDDD" class="style8a"><div align="center">Period Completed<br />
                              (Y-M-D)</div></td>
                            <td width="12%" rowspan="2" bgcolor="#DDDDDD" class="style8a">&nbsp;</td>
                          </tr>
                          <tr>
                            <td width="8%" bgcolor="#DDDDDD" class="style8a"><div align="center">From</div></td>
                            <td width="8%" bgcolor="#DDDDDD" class="style8a"><div align="center">To</div></td>
                          </tr>
                        <?php
				   if($cnts>0)
				   {
...
//								echo "<br>dd=".$fd."--".$td."--".$fdt."--".$tdt."       ".$diff;
						...
						?>
                          <tr bgcolor="<?php echo $bg ?>">
                            ...
                            <td><div align="center">
                              <?php if($days!='invalid') echo $days[0]."-".$days[1]."-".$days[2]; ?>
                            </div></td>
                  ...
                  <?php
				   ...
				   ?>
                      ...
                      <tr bgcolor="#FFFFFF">
                        <td colspan="3" class="style8"><table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#FFFFFF" bgcolor="#FFFFFF">
					<?php
//						if($rowp["E_DES_1"]!=NULL)
//						{
					?>
                          <tr bgcolor="#DDDDDD">
                            <td width="6%" class="style8a"><div align="center">S.No</div></td>
                            <td width="21%" class="style8a"><div align="center">Designation After Promotion</div></td>
                            <td width="18%" class="style8a"><div align="center">Date Of Promotion</div></td>
                            <td width="19%" class="style8a"><div align="center">Type Of Promotion<br />
                              (Regular/ACP/MACP)</div></td>
                            <td width="19%" class="style8a"><div align="center">Order No.</div></td>
                            <td width="19%" class="style8a">&nbsp;</td>
                          </tr>
                        <?php
							$p=1;
							$b=1;
...
//								{
									...
									if(!empty($_POST['E_DOP_DT'.$p]))
									$edt=$_POST['E_DOP_DT'.$p];
									else if($rowp['E_DOP_DT']!="")
						...
						?>
                          <tr bgcolor="<?php echo $bg ?>">
                            ...
                            <td><div align="center">
                              <label>
                              <input type="date" name="E_DOP_DT<?php echo $p ?>" id="E_DOP_DT<?php echo $p ?>" value="<?php echo $edt; ?>" />
                              </label>
                            </div></td>
							   <?php
								$dates1.=",E_DOP_DT".$p;
				   ...
				   $vn=1;
					?>
                          <tr bgcolor="#DDDDDD">
                            ...
                            <td width="21%"><div align="center">Case Ref. No.</div></td>
                            <td width="18%"><div align="center">Details Of The Case</div></td>
                            <td width="19%"><div align="center">Status Of The Case</div></td>
                            <td width="19%">&nbsp;</td>
                          </tr>
                          <?php
							$v=1;
...
//								{
									...
									if(!empty($_POST['refno'.$v]))
									$refno=$_POST['refno'.$v];
									else if($rowd['refno']!="")
									$refno=$rowd['refno'];
									else $refno="";

									if(!empty($_POST['det'.$v]))
									$det=$_POST['det'.$v];
									else if($rowd['det']!="")
									$det=$rowd['det'];
									else $det="";

									if(!empty($_POST['status'.$v]))
...
//						}
				 ...
				 ?>
					...
					?>
                      ...
                      <tr>
                        <td colspan="3" bgcolor="#FFFFFF" class="style8a"><table width="100%" border="1" cellpadding="4" cellspacing="1" bordercolor="#FFFFFF" bgcolor="#FFFFFF" class="style8a">
                            <?php
				   ...
				   $ln=1;
					?>
                            <tr>
                              <td width="6%" rowspan="2" bgcolor="#DDDDDD" class="style8"><div align="center">S.No</div></td>
                              <td width="18%" rowspan="2" bgcolor="#DDDDDD" class="style8"><div align="center">Date</div></td>
                              <td colspan="5" bgcolor="#DDDDDD" class="style8"><div align="center">Earned</div></td>
                              <td colspan="5" bgcolor="#DDDDDD" class="style8"><div align="center">HPL</div></td>
                              <td width="19%" rowspan="2" bgcolor="#DDDDDD" class="style8">&nbsp;</td>
                            </tr>
                            <tr>
                              <td width="6%" bgcolor="#DDDDDD" class="style8">Opening Balance</td>
                              <td width="7%" bgcolor="#DDDDDD" class="style8">Accured</td>
                              <td width="6%" bgcolor="#DDDDDD" class="style8">Total</td>
                              <td width="6%" bgcolor="#DDDDDD" class="style8">Used</td>
                              <td width="6%" bgcolor="#DDDDDD" class="style8">Balance</td>
                              <td width="6%" bgcolor="#DDDDDD" class="style8">Opening Balance</td>
                              <td width="6%" bgcolor="#DDDDDD" class="style8">Accured</td>
                              <td width="4%" bgcolor="#DDDDDD" class="style8">Total</td>
                              <td width="4%" bgcolor="#DDDDDD" class="style8">Used</td>
                              <td width="6%" bgcolor="#DDDDDD" class="style8">Balance</td>
                            </tr>
                            <?php
							$l=1;
				   ...
				   if($cntl>0)
				   {
							while($rowl=@mysqli_fetch_array($seldl))
							{
									...
									if(!empty($_POST['eob'.$l]))
									$eob=$_POST['eob'.$l];
									else if(!empty($rowl['eob']))
									$eob=$rowl['eob'];
									else 
									$eob="";
									
									if(!empty($_POST['eaccured'.$l]))
									$eaccured=$_POST['eaccured'.$l];
									else if(!empty($rowl['eaccured']))
									$eaccured=$rowl['eaccured'];
									else 
									$eaccured="";
									
									if(!empty($_POST['eused'.$l]))
									$eused=$_POST['eused'.$l];
									else if(!empty($rowl['eused']))
									$eused=$rowl['eused'];
									else 
									$eused="";
									
									if(!empty($_POST['hob'.$l]))
									$hob=$_POST['hob'.$l];
									else if(!empty($rowl['hob']))
									$hob=$rowl['hob'];
									else 
									$hob="";
									
									if(!empty($_POST['haccured'.$l]))
									$haccured=$_POST['haccured'.$l];
									else if(!empty($rowl['haccured']))
									$haccured=$rowl['haccured'];
									else 
									$haccured="";
									
									if(!empty($_POST['hused'.$l]))
									$hused=$_POST['hused'.$l];
									else if(!empty($rowl['hused']))
									$hused=$rowl['hused'];
									else 
									$hused="";
						...
						?>
                            <tr bgcolor="<?php echo $bg ?>">
                              <td bgcolor="#DDDDDD" class="style8"><?php echo $l ?>
                                  <input type="hidden" name="lid<?php echo $l ?>" id="lid<?php echo $l ?>" value="<?php echo $rowl['id'] ?>" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="edate<?php echo $l ?>" type="date" id="edate<?php echo $l ?>" value="<?php echo $edate; ?>" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="eob<?php echo $l ?>" type="text" id="eob<?php echo $l ?>" size="5" value="<?php echo $eob; ?>" onchange="cal_leavtot('e','<?php echo $l ?>');" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="eaccured<?php echo $l ?>" type="text" id="eaccured<?php echo $l ?>" size="5" value="<?php echo $eaccured; ?>" onchange="cal_leavtot('e','<?php echo $l ?>');" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="etot<?php echo $l ?>" type="text" id="etot<?php echo $l ?>" size="10" value="<?php echo $refno; ?>" readonly="readonly" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="eused<?php echo $l ?>" type="text" id="eused<?php echo $l ?>" size="5" value="<?php echo $eused; ?>" onchange="cal_leavtot('e','<?php echo $l ?>');" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="ebal<?php echo $l ?>" type="text" id="ebal<?php echo $l ?>" size="5" value="<?php echo $refno; ?>" readonly="readonly" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="hob<?php echo $l ?>" type="text" id="hob<?php echo $l ?>" size="5" value="<?php echo $hob; ?>" onchange="cal_leavtot('h','<?php echo $l ?>');" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="haccured<?php echo $l ?>" type="text" id="haccured<?php echo $l ?>" size="5" value="<?php echo $haccured; ?>" onchange="cal_leavtot('h','<?php echo $l ?>');" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="htot<?php echo $l ?>" type="text" id="htot<?php echo $l ?>" size="10" value="<?php echo $refno; ?>" readonly="readonly" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="hused<?php echo $l ?>" type="text" id="hused<?php echo $l ?>" size="5" value="<?php echo $hused; ?>" onchange="cal_leavtot('h','<?php echo $l ?>');" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><input name="hbal<?php echo $l ?>" type="text" id="hbal<?php echo $l ?>" size="5" value="<?php echo $refno; ?>" readonly="readonly" /></td>
                              <td bgcolor="#DDDDDD" class="style8"><div align="center"><a style="cursor:pointer" onclick="deltvl('<?php echo $rowl['id'] ?>')">&nbsp;Delete&nbsp;</a></div></td>
                            </tr>
                            <script>
...
var dsuc='<?php echo $_GET['dsuc'] ?>';
var dpsuc='<?php echo $_GET['dpsuc'] ?>';
var dvsuc='<?php echo $_GET['dvsuc'] ?>';
var dlsuc='<?php echo $_GET['dlsuc'] ?>';
