...
$seldat1=executework("select tdate from tob_auction_2209 where isactive=1 and platf in(select id from tob_platform where state='".$stat."')".$qry." order by tdate limit 1");
//echo "select tdate from tob_auction_2209 where isactive=1 and platf in(select id from tob_platform where state='".$stat."')".$qry." order by tdate limit 1";
...
if(!empty($pdat))
{
	...
	if(!empty($_POST['subm']))
	{
		$selmax=executework("select max(id)  from tob_auction_2209");
		...
		{
			$tval=0;
			$tval=$_POST['aprice'.$i]*$_POST['qsold'.$i];
			$selsum=executework("select sum(qsold) as qs,sum(tvalue) as tv from tob_auction_2209 where tdate<'".$dates."' and platf=".($_POST['fid'.$i]));
			...
			$cavg=0;
			$selrec=executework("select * from tob_auction_2209 where tdate='".$dates."' and platf=".($_POST['fid'.$i]));
			$cnt=@mysqli_num_rows($selrec);
			if($cnt>0)
			{
				$rowm=@mysqli_fetch_array($selrec);
				$uprec=executework("update tob_auction_2209 set apf=".set_num(($_POST['apf'.$i])).",bales=".set_num(($_POST['bales'.$i])).",ns=".set_num(($_POST['ns'.$i])).",nb=".set_num(($_POST['nb'.$i])).",rr=".set_num(($_POST['rr'.$i])).",cr=".set_num(($_POST['cr'.$i])).",tot=".set_num(($_POST['tot'.$i])).",bsold=".set_num(($_POST['bsold'.$i])).",qsold=".set_num(($_POST['qsold'.$i])).",aprice=".set_num(($_POST['aprice'.$i])).",cprice=".set_num($cavg).",tvalue=".$tval." where id=".($rowm['id']));
				//echo "<br>update tob_auction_2209 set apf=".set_num(($_POST['apf'.$i])).",bales=".set_num(($_POST['bales'.$i])).",ns=".set_num(($_POST['ns'.$i])).",nb=".set_num(($_POST['nb'.$i])).",rr=".set_num(($_POST['rr'.$i])).",cr=".set_num(($_POST['cr'.$i])).",tot=".set_num(($_POST['tot'.$i])).",bsold=".set_num(($_POST['bsold'.$i])).",qsold=".set_num(($_POST['qsold'.$i])).",aprice=".set_num(($_POST['aprice'.$i])).",cprice=".set_num($cavg).",tvalue=".$tval." where id=".($rowm['id']);
			}
			else
			{
				if(empty($_POST['cdate'.$i]))
				$_POST['cdate'.$i]='00-00-0000';
				if(empty($_POST['edate'.$i]))
				$_POST['edate'.$i]='00-00-0000';
				$intrec=executework("insert into tob_auction_2209 values(".$maxid.",".$ndate1.",'".$dates."',".($_POST['fid'.$i]).",".set_num(($_POST['apf'.$i])).",".set_num(($_POST['aqty'.$i])).",".set_num(($_POST['eqty'.$i])).",'".datepattrn1(($_POST['cdate'.$i]))."','".datepattrn1(($_POST['edate'.$i]))."',".set_num(($_POST['bales'.$i])).",".set_num(($_POST['ns'.$i])).",".set_num(($_POST['nb'.$i])).",".set_num(($_POST['rr'.$i])).",".set_num(($_POST['cr'.$i])).",".set_num(($_POST['tot'.$i])).",".set_num(($_POST['bsold'.$i])).",".set_num(($_POST['hbid'.$i])).",".set_num(($_POST['lbid'.$i])).",".set_num(($_POST['buyers'.$i])).",".set_num(($_POST['qsold'.$i])).",".set_num(($_POST['aprice'.$i])).",".set_num($cavg).",".$tval.",1)");
				//echo "insert into tob_auction_2209 values(".$maxid.",".$ndate1.",'".$dates."',".$_POST['fid'.$i].",".set_num($_POST['apf'.$i]).",".set_num($_POST['aqty'.$i]).",".set_num($_POST['eqty'.$i]).",'".datepattrn1($_POST['cdate'.$i])."','".datepattrn1($_POST['edate'.$i])."',".set_num($_POST['bales'.$i]).",".set_num($_POST['ns'.$i]).",".set_num($_POST['nb'.$i]).",".set_num($_POST['rr'.$i]).",".set_num($_POST['cr'.$i]).",".set_num($_POST['tot'.$i]).",".set_num($_POST['bsold'.$i]).",".set_num($_POST['hbid'.$i]).",".set_num($_POST['lbid'.$i]).",".set_num($_POST['buyers'.$i]).",".set_num($_POST['qsold'.$i]).",".set_num($_POST['aprice'.$i]).",".set_num($cavg).",".$tval.",1)";
				$maxid++;
			}
		}
		$succ=1;
...
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" enctype="multipart/form-data" name="form1" id="form1" onsubmit="return check(this);">
...
if(!empty($pdat) && !empty($stat))
	{
	//echo df;
	$sel=executework("select distinct tdate,state from tob_auction_2209,tob_platform where tob_auction_2209.tdate='".(datepattrn1($pdat))."' and tob_platform.state='".$stat."' order by tob_platform.catg");
...
<?php
	...
	if(!empty($stat))
	{
		$sel=executework("select distinct catg from tob_platform where state='".($stat)."' and isactive=1 order by field(catg,'NBS','SBS','SLS','NLS'),seqid,sno");
	...
	<?php
		...
		while($rowd=@mysqli_fetch_array($sel))
		{
			...
			while($row=@mysqli_fetch_array($seldet))
			{
				...
				$selm=executework("select * from tob_auction_2209 where platf=".$row['id']." and tdate ='".datepattrn1($pdat)."' order by tdate desc limit 1 ");
				//echo ("select * from tob_auction_2209 where platf=".$row['id']." and tdate ='".datepattrn1($pdat)."' order by tdate desc limit 1 ");
				$cntm=@mysqli_num_rows($selm);
				
				$selr=executework("select apf,aqty,eqty,cdate,edate from tob_auction_2209 where platf=".$row['id']." and tdate<'".datepattrn1($pdat)."' order by tdate desc limit 1 ");
				...
				$sel_tprice=executework("select SUM(tvalue) as tv from tob_auction_2209 where tdate <= '".datepattrn1($pdat)."' and platf=".$row['id']." ");
				
				//echo ("select SUM(tvalue) as tv from tob_auction_2209 where tdate <= '".datepattrn1($pdat)."' and platf=".$row['id']." ");
				$row_tprice=@mysqli_fetch_array($sel_tprice);
				//echo "tprice=".$row_tprice['tv'];
				
				$sel_qsold=executework("select SUM(qsold) as qs from tob_auction_2209 where tdate <= '".datepattrn1($pdat)."' and  platf=".$row['id']." ");
				
				//echo ("select SUM(qsold) as qs from tob_auction_2209 where tdate <= '".datepattrn1($pdat)."' and  platf=".$row['id']." ");
				$row_qsold=@mysqli_fetch_array($sel_qsold);
				
				$cummulative_price=$row_tprice['tv']/$row_qsold['qs'];
				//echo "hiii".$cummulative_price;
	?>
        <tr>
          ...
          <td bgcolor="#FFFFFF"><label><input name="cprice<?php echo $i ?>" type="text" id="cprice<?php echo $i ?>" value="<?php if(!empty($cummulative_price)) echo numround($cummulative_price,2) ?>" readonly="true" />
          </label>
