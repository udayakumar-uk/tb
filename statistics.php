...
<div id="main-content">
  <div id="content" class="container">
    ...
    if(!empty($_GET['stype']))
      $archive.=" and stype='".$stype."'";
    
    $select=executework("select * from tob_statistics ".$archive." order by id desc");
	...
	$select1= executework("select * from tob_statistics $archive order by id desc LIMIT $pagenow1, $max_recs_per_page");
	$count1 = @mysqli_num_rows($select1);
	

if($pages > 1) { ?>
      <div class="d-flex align-items-center justify-content-between">
        <h4 class="sub-title no-dash mb-0">Statistics</h4>
        <ul class="pagination">
    <?php
	    for($im=1;$im<=$pages;$im++) {
            ...
            ?>
            <li class="page-item"><a class="page-link hlink1" href="statistics.php?page_index=<?php echo $im; ?>&amp;stype=<?php echo $q_stype; ?>&amp;ar=<?php echo $q_ar; ?>"> <?php echo $im; ?> </a></li>
          <?php } else { ?>
    ...
    <ul>
       <?php while($rows=@mysqli_fetch_array($select1)) {
				if($rows['tfile']!="")
				$link=$base_url."/tbdata/statisticsfiles/".$rows['tfile'];
				else
				$link="#";
        ...
        <li><strong><?php echo $rows['title'];?></strong><br />
          <a href="<?php echo $link?>" target="<?php if($link!="#"){?>_blank<?php }?>"><?php echo $rows['description']; if($fcheck[1]=='pdf'){?> <img src="tob2_imgs/pdf_icon.gif" width="12" height="15" border="0" /><?php }?></a> 
          <a href="<?php echo $link?>" target="<?php if($link!="#"){?>_blank<?php }?>"><img src="tob2_imgs/newindow_icon.gif" border="0" /></a>
        </li>
          <?php } else { echo "<li><strong>".$rows['title']."</strong><br />".$rows['description']."</li>"; } } ?>
    </ul>
