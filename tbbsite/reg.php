...
function createZipAndDownload($files, $filesPath, $zipFileName)
{
	...
	$zipFileName=$filesPath."/".$zipFileName;
    $zip = new \ZipArchive();
...
if($_POST['email']!="")
{
	...
	$dat=date('Y-m-d');
		...
		$intreg=executework("insert into tob_reg values(".$maxid.",".$_POST['id'].",'".$dat."','".$_POST['name']."','".$_POST['adrs']."','".$_POST['email']."','".$_POST['phno']."','".$_POST['faxno']."','".$_POST['website']."','".$_POST['cpersion']."','".$_POST['designation']."','".$_POST['mobnum']."')");
		 
		// $sel=executework("select * from tob_tender where id='".$tid."'");
//		 echo ("select * from tob_tender where id='".$tid."'");
//		 exit();
//		 $row=@mysqli_fetch_array($sel);
		 
	//	redirect("tbdata/tenderfiles/".$row['mfile']);
	//	redirect("tbdata/tenderfiles/".$row['tfile']);
	//	redirect("tbdata/tenderfiles/".$row['sfile']);
	
		 $filename   =   'Tenders.zip';
    	 $sel=executework("select * from tob_tender where id='".$tid."'");
     	 $row=@mysqli_fetch_array($sel);
		 $arr=array($row['mfile'],$row['tfile'],$row['sfile']);
...
//	   echo $arr[$i];
//	   //exit();
//            $zip->addFile(getcwd().'/'.'tbdata/tenderfiles/'.$arr[$i], $arr[$i]);
//			echo getcwd().'/'.'tbdata/tenderfiles/'.$arr[$i];
//			//exit();
//        }
//        $zip->close();
//         
...
//    }
		...
		$files = array($row['mfile'],$row['tfile'],$row['sfile']);
	
// Directory of files
$filesPath = 'tbdata/tenderfiles/';
// Name of creating zip file
$zipName = 'Tenders.zip';
	if(file_exists($filesPath.$zipName))
		unlink($filesPath.$zipName);
