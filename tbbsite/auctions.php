...
while($row=@mysqli_fetch_array($selplat))
{
	...
	$selqdat1=executework("SELECT year FROM tob_auction and platf=".$row['id'].$qry1." order by year desc limit 1");
...
Cumulative Information </td>
				  ...
				  ?>
						...
						?>
							...
							//$qrv=" and tdate='".$adate."'";
							$sel1=executework("select distinct platf,platform from tob_auction,tob_platform where tob_auction.platf=tob_platform.id and state='".$stat."'".$qrv." and tob_platform.isactive=1 order by field(tob_platform.catg,'NBS','SBS','SLS','NLS'),tob_platform.seqid");
