...
<section id="content">
	<div class="zerogrid">
		...
		<section class="col-1-3" style="width:35%">
		...
		<section class="col-1-3" style="width:33%">
			<!--	<h2 style="border-bottom:2px solid #c66f2f">CIRCULARS & NOTIFICATIONS</h2>-->
				<div class="content" style="padding-top:1px; text-align:justify">
						...
						<div style="font-size:13px; background-color:#D8F5E7; padding:7px; border-radius:5px;"><strong>Mission:</strong> "To strive for the overall development of tobacco growers and the Indian Tobacco Industry."</div>

                      <div style="font-size:13px; background-color: #FFDFE6;  padding:7px; border-radius:5px; margin-top:7px;"> <strong> Vision:</strong> "Tobacco Board is committed to accomplishing its role - the expressed will of Parliament - for the smooth functioning of a vibrant farming system, fair and remunerative prices to tobacco growers and export promotion." </div>
					</div>
			...
			<section class="col-1-3" style="width:26%; margin-top:-40px;">
				<!--<h2 style="border-bottom:2px solid #c66f2f">WHATS NEW</h2>-->
				<div class="content">
						<!--<p><a class="twitter-timeline" data-width="250" data-height="200" href="https://twitter.com/TobaccoBoard?ref_src=twsrc%5Etfw">Tweets by TobaccoBoard</a> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script> </script> -->
                        
                        <h2 style="border-bottom:2px solid #c66f2f; margin-top:0px;">Photogallery</h2>
                        <marquee direction="left" onMouseOver="this.stop();" onMouseOut="this.start();" loop>
                        <?php 
				  while($rowp=@mysqli_fetch_array($selectp))
				  {
				  	if(!empty($rowp['image']) && file_exists("tbdata/photogallery/oimages/".$rowp['image']))
					{
				  ?>
				<a href="viewphoto.php?tit=<?php echo urlencode($rowp['title']); ?>"><img src="http://tobaccoboard.com/tbdata/photogallery/oimages/<?php  echo $rowp['image'] ?>" style="height:180px; width:auto" height="180" width="auto"></a>
                   <?php 
				   	}
				  }
				  ?>    
...
?>
  ...
  {
  ?>
				<h2 style="border-bottom:2px solid #c66f2f;margin-top:0px;"><?php echo $states1[$i]; ?> AUCTION PRICE</h2>
          <table width="100%" border="0">
  <tr style="background-color: #292860; color: white; text-align: center;">
    <td style="border:solid 1px"><strong>Catg</strong></td>
    ...
    <td style="border:solid 1px"><strong>Qty<br>
      (Kgs)</strong></td>
    <td style="border:solid 1px"><strong>Cumm.<br>
      Price</strong></td>
  </tr>
  <?php
  	for($j=0;$j<count($catgs[$i]);$j++)
	{
		$sel3=executework("select * from tob_auction,tob_platform where tob_auction.platf=tob_platform.id and catg='".$catgs[$i][$j]."' and date(tdate)<='".date('Y-m-d')."' and tob_platform.isactive=1 order by tob_auction.tdate desc limit 1");
		$row=@mysqli_fetch_array($sel3);
		$adate=$row['tdate'];
		$yr=$row['year'];
		$selycnt=executework("select distinct count(distinct tdate) as cnt from tob_platform,tob_auction where tob_platform.id=tob_auction.platf and tob_platform.catg='".$catgs[$i][$j]."' and date(tdate)<='".date('Y-m-d')."' and tob_auction.year=".$yr." order by cnt desc limit 1");
		$rowycnt=@mysqli_fetch_array($selycnt);
		if(!empty($rowycnt['cnt']))
		$days=$rowycnt['cnt'];
	
		$qrv=" and year ='".$yr."'";
		$selsm=executework("select sum(bsold) as bsold,sum(qsold) as qsold,sum(tvalue) as tval,sum(aprice) as apric from tob_auction where platf in(select id from tob_platform where catg='".$catgs[$i][$j]."')".$qrv);
  ...
  ?>
  <tr>
    <td style="border:solid 1px"><?php echo $catgs1[$i][$j]; ?></td>
...
?>
	  ...
	  <section class="col-lg-4" style="width:25%">
				<h2 style="border-bottom:2px solid #c66f2f;margin-top:0px;">STATISTICS</h2>
			  <a href="export_per.php"> <div id="chart2" style="width:210px; height:175px;"></div></a>			</section>
      ...
      <div id="chart3" style="width:215px; height:170px;margin-top:10px" ></div>
            ...
            <section class="col-lg-4" style="width:25%">
				<h2 style="border-bottom:2px solid #c66f2f;margin-top:0px;">ANNUAL REPORTS</h2>
				<div class="content">
						<a href="auctions1.php">
      <div id="chart3" style="width:215px; height:170px;margin-top:10px" ></div>
      </a> 
