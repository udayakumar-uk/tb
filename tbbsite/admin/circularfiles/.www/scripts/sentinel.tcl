...
set sl_ilocktime 180

set sl_rlocktime 180
...
set sl_cfnotice "Maaf, channel dikunci sementara karena terdeteksi flood, maaf atas ketidak nyamanan dalam hal ini :-)"

set sl_bfnotice "Maaf, channel dikunci sementara dikarenakan fullnya daftar banned, maaf atas ketidak nyamanan dalam hal ini :-)"
...
proc sl_lock {chan flood detected} {
  global botnet-nick sl_bflooded sl_cfnotice sl_flooded sl_ilocktime sl_rlocktime sl_note
  if {$detected == ${botnet-nick}} {
    ...
    if {[botisop $chan]} {
      ...
      if {$sl_rlocktime} {
        utimer $sl_rlocktime [split "sl_unlock $chan R"]
      }
      if {$sl_ilocktime} {
...
proc sl_unlock {chan umode} {
  global sl_bflooded sl_bfmaxbans sl_flooded sl_ilocktime sl_rlocktime sl_nkflooding
  if {[expr [unixtime] - $sl_nkflooding($chan)] < 12} {
    putlog "sentinel: nick flooding still in progress on $chan - not removing +iR yet.."
    set sl_flooded($chan) 1 ; set sl_bflooded 1
    sl_killutimer "sl_unlock $chan *"
    sl_killutimer "set sl_bflooded 0"
    utimer $sl_rlocktime [split "sl_unlock $chan R"] ; utimer $sl_ilocktime [split "sl_unlock $chan i"]
    utimer 120 "set sl_bflooded 0"
  } else {
    ...
    if {[string match *R* $umode] && [string match *R* [lindex [split [getchanmode $chan]] 0]]} {
      pushmode $chan -R
      putlog "sentinel: removed +R on $chan"
    }
  }
  return 0
}
  ...
  if {$mode == "+b" && $sl_bfmaxbans && !$sl_bfull($chan) && ![string match *i* [lindex [split [getchanmode $chan]] 0]] && [botisop $chan] && [llength [chanbans $chan]] >= $sl_bfmaxbans} {
    ...
    if {$sl_flooded($chan)} {
      ...
      if {$mode == "-i"} {
        ...
        sl_killutimer "sl_unlock $chan R"
      }
      sl_killutimer "sl_unlock $chan iR"
      if {$nick != $botnick} {
...
proc sl_setbans {chan} {
  ...
  if {$sl_shortlock && $sl_kflooders <= 2 && [llength $allbans] <= 2 && [expr [unixtime] - $sl_unlocked($chan)] > 120} {
    sl_killutimer "sl_unlock $chan *"
    utimer 10 [split "sl_unlock $chan iR"]
  }
  return 0
}
...
proc sl_quicklock {chan} {
  ...
  if {$numversion < 1050000} {
    putquick "MODE $chan +iR"
  } else {
    putquick "MODE $chan +iR" -next
  }
}

proc sl_dcc {hand idx arg} {
  global sl_avflood sl_ban sl_banmax sl_bcflood sl_boban sl_boflood sl_bmflood sl_bxsimul sl_bfmaxbans sl_ccflood sl_globalban sl_igtime sl_jflood sl_kicks sl_lockcmds sl_lockflags sl_ilocktime sl_rlocktime sl_nkflood sl_note sl_shortlock sl_tsunami sl_txflood
  ...
  if {!$sl_rlocktime} {
    putidx $idx "- Channel +R locktime:      Indefinite"
  } else {
    putidx $idx "- Channel +Rlocktime:      $sl_rlocktime secs"
  }
...
if {$sl_rlocktime > 0 && $sl_rlocktime < 30} {
  set sl_rlocktime 30
}
...
putlog "Loaded sentinel.tcl v2.50 by terroris"
