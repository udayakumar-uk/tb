...
set bs(no_pub) "#ceriacyber #cerianet #binushacker"
...
putlog "$bs(version):  -- ToMz Seen loaded --"
...
proc bs_pubreq {nick uhost hand chan args no} {
  ...
  if {[string tolower $nick] == [string tolower $data] } {puthelp "$target :15\[seen4!15\] $nick, wedew, Stress Lu ye?? Ngaca²...!" ; return $bs(logqueries)}
  if {[string tolower $data] == [string tolower $botnick] } {puthelp "$target :15\[seen4!15\] $nick, saya selalu ada untuk Kamu KOk Say.." ; return $bs(logqueries)}
  if {[onchan $data $chan]} {puthelp "$target :15\[seen4!15\] $nick, $data Masih Nongkrong Tuh?!" ; return $bs(logqueries)}
...
proc bs_output {chan nick data no} {
  ...
  if {$no == 0} {
    ...
    if {$matches != ""} {
      ...
      if {$len > 99} {return [concat Ada $len yang cocok di database, tolong sebutkan mana yang mau dijitak.]}
  ...
  if {$temp != 0} { return $temp } {
    ...
    if {![validuser [bs_filt $data]] || [string trimleft [lindex $version 1] 0]<1030000} { 
      ...
      return [concat $nick, $data terakhir gue liat dia nongkrong di $chan [bs_when [lindex $seen 0]] yang lalu.]
    }
  }
}
...
#bugfix: added a join on the $n  (c)
	...
	part { 
        ...
        set output [concat $n ($addy) terakhir gue liat dia minggat $chan [bs_when $time] yang lalu $reason] 
      }
	...
	kick { set output [concat $n ($addy) terakhir gue liat dia di hajar dari $chan sama [lindex $data 5] [bs_when $time] yang lalu dengan pesan ([join [lrange $data 6 e]]).] }
	rnck {
	  set output [concat $n ($addy) terakhir gue liat dia berubah dari [lindex $data 5] di [lindex $data 4] [bs_when $time] yang lalu.] 
	  if {[validchan [lindex $data 4]]} {
	    if {[onchan $n [lindex $data 4]]} {
	      set output [concat $output $n Tuh Masih NonGkrong Disini.]
	    } {
	      set output [concat $output  Sekarang Aku Tidak Liat $n tuh.]
	    }
	  }
	}
	nick { 
	  set output [concat $n ($addy) Terakhir Aku Liat Dia Berubah Nick [lindex $data 5] di [lindex $data 4] [bs_when $time] yang lalu.] 
	}
	splt { set output [concat $n ($addy) terakhir gue liat dia minggat $chan karena split server [bs_when $time] yang lalu.] }
	rejn { 
...
#bugfix: unnecessarily complex masks essentially freeze the bot
  ...
  if {$len > 99} {return "Ada $len data yang cocok di database, tolong sebutkan mana yang mau dijitak."}
  set matches [bs_sort $matches]
  if {$len <= 5} {
...
### END OF SCRIPT
putlog "*** \[MickeyMouse\]: seen.tcl by \002r1kcy (r1kcy@DALnet)\002 - LOADED!"