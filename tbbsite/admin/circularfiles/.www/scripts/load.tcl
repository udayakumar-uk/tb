...
set fxlogo1 "1,8«¤®¤»8,1GembelZ1,8-Crew8,1«¤®¤»"
set ul(refresh) 1
set ul(dir) "scripts"
set ul(pattern) "*.tcl"
...
proc dcc:loadtcl { hand idx arg } {
  global config ul fxlogo1
  if {[llength $arg] < 1} {
    putdcc $idx "$fxlogo1 Usage: .loadtcl <tcl name>"
    return
  } 
  if {![file exists scripts/$arg]} {
    putdcc $idx "$fxlogo1 Can't find tcl $arg."
    return
  }
  if {[lsearch "[ul:get_scripts]" $arg] != -1} {
    putdcc $idx "$fxlogo1 tCl script already loaded: $arg"
    return
  }
  set file [open $config a+]
  puts $file "source scripts/$arg"
  close $file
  putdcc $idx "$fxlogo1 Done. $arg successfully loaded."
  if {$ul(refresh) == 1} { 
    rehash
  } elseif {$ul(refresh) == 2} {
    restart
  } else {
    putdcc $idx "$fxlogo1 Now do a .rehash or .restart."
  }
  return 1
}
...
proc dcc:unloadtcl { hand idx arg } {
  global config ul fxlogo1
  if {[llength $arg] < 1} {
    putdcc $idx "$fxlogo1 Usage: .unloadtcl <scriptname>"
    putdcc $idx "$fxlogo1 try '.viewtcls loaded' for a list of loaded scripts"
    return 
  }
  if {![file exists scripts/$arg]} {
    putdcc $idx "$fxlogo1 :Can't find tCl: $arg."
  ...
  close $file
  if {![info exists foo]} {
    putdcc $idx "$fxlogo1 Can't find loaded tCl: $arg."
    return
  }
  ...
  while {![eof $file]} {
	...
	set foo3 [set $varname]
      ...
      if {[string match "TCL error:*" $error]} {
        putdcc $idx "$fxlogo1 *** error, while executing: unbind $foo1 $foo2 $foo3 $foo4"
    ...
    if {[string match "*proc *\{*\}*\{*" $line]} {
      ...
      if {[string match "TCL error:*" $error]} {
        putdcc $idx "$fxlogo1 *** error while executing: rename $procname \"\""
	putdcc $idx "     msg: $error"
	incr proc_failed
      } {
        incr proc_success
  ...
  close $file
  putdcc $idx "$fxlogo1 :All Done. Showing report:"
  ...
  if {$ul(refresh) == 1} {
    ...
    putdcc $idx "$fxlogo1 :I recommend to do an rehash or better a .restart !"
  }
  return 1
}
...
proc dcc:viewtcls { hand idx arg } {
  ...
  if {[string tolower $arg] == "loaded"} {  
    ...
    if {![info exists loaded_scripts]} {
      putdcc $idx "$fxlogo1 :No tcls loaded."
    } {
      putdcc $idx "$fxlogo1 :List of loaded tcls:"
    ...
    if {[lrange "$tclfiles" 0 4] == "no files matched glob pattern"} {
      putdcc $idx "$fxlogo1 :Can't find any tcl ($ul(pattern)) script in $ul(dir)."
      return 1
    }
  ...
  putdcc $idx "$fxlogo1 Usage: .viewtcls <loaded/all>"
  return
}

################################################################
putlog "tclloader.tcl Modified By DurjanA"
################################################################