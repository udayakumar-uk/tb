...
<html lang="en">
<head>
	<title> Circulars | Tobacco Board</title>
  ...
  <div id="content" class="container">
    ...
    if($pages > 1) { ?>
      <div class="d-flex align-items-center justify-content-between">
        <h4 class="sub-title no-dash mb-0">Circulars</h4>
        <ul class="pagination">
    <?php
	    for($im=1;$im<=$pages;$im++) {
            ...
            ?>
            <li class="page-item"><a class="page-link hlink1" href="circulars.php?page_index=<?php echo $im; ?>&amp;stype=<?php echo $q_stype; ?>&amp;ar=<?php echo $q_ar; ?>"> <?php echo $im; ?> </a></li>
          <?php } else { ?>
    ...
    <ul>
       <?php while($rows=@mysqli_fetch_array($select1)) {
				if($rows['tfile']!="")
				$link=$base_url."/tbdata/circularfiles/".$rows['tfile'];
				else
				$link="#";
