...
$selplata1=executework("select * from tob_platform where state='".$stat1."' and isactive=1 order by platform");
$cntpa1=@mysqli_num_rows($selplata1);
$u=0;
while($rowa1=@mysqli_fetch_array($selplata1))
{
	$plata1[$u]=array($rowa1['id'],$rowa1['platform']);
	$u++;
}
...
$selplata=executework("select * from tob_platform where state='".$stat1."' and isactive=1".$qry." order by platform");
$cntpa=@mysqli_num_rows($selplata);
$t=0;
$str="";
$stra="";
...
while($row=@mysqli_fetch_array($selplata))
{
	$plat[$t]=array($row['id'],$row['platform']);
		$selqdat=executework("select tdate,aprice,cprice,qsold from tob_platform,tob_auct,tob_auction where tob_platform.id=tob_auction.platf and tob_auct.platf=tob_auction.platf and tob_auct.year=tob_auction.year and tob_platform.id=".$row['id'].$qry1.$dt." order by tob_auction.tdate desc");
		//echo "<br>select tdate,aprice,cprice,qsold from tob_platform,tob_auct,tob_auction where tob_platform.id=tob_auction.platf and tob_auct.platf=tob_auction.platf and tob_auct.year=tob_auction.year and tob_platform.id=".$row['id'].$qry1.$dt." order by tob_auction.tdate desc";
//	$selqdat=executework("select tdate,aprice,qsold from tob_platform,tob_auction where tob_platform.id=tob_auction.platf and tob_platform.id=".$row['id']." and tob_auction.year=".$yr.$qry1.$dt." order by tob_auction.tdate");
	...
	if($cntd>0)
	{
		...
		if($stra=="")
		$stra=$row['platform']."*".$st1."*".$st2."*".$st3;
		else
		$stra=$stra."~".$row['platform']."*".$st1."*".$st2."*".$st3;
		
		if(empty($sta))
		$sta=$row['platform'];
		else
		$sta=$sta.":".$row['platform'];
...
color:#0c6f0c;font-size:30px;"><a href="index.php">E-AUCTIONS</a></h4>
      ...
      <div class="row">
        <div class="col-lg-6">
           ...
           <div class="card">
		    ...
		    <h6 class="text-uppercase">DAY INFORMATION (<?php echo strtoupper($stat); ?>)</h6>
              ...
              <div class="card-body" style="padding: 0.25em;"> 
                ...
                <!-- Tab panes -->
                <div class="tab-content" style="padding: 0em;">
                  <div id="piil-1" class="container tab-pane active" style="padding: 0px;">
                     <form id="actn" name="actn" method="post" action="">
                         
                          <div class="example-content">
                            
							<table border="0" style="width: 565px;">
                              <tr>
                                <td style="vertical-align: top"><div align="center"><!-- Example scripts go here -->
                            <div id="chart2" style="margin-top:20px; margin-left:20px; width:550px; height:500px;" align="center"></div></div></td>
              ...
              <table id="" class="table table-bordered" style="font-size: 12px;">
                <thead>
                    <tr style="color:#008cff;">
                      <th colspan="6" style="padding: 0.25em;"><?php echo $yr; ?> Year Auctions - Sales Report</th>
                      </tr>
                    <tr style="color:#008cff; padding: 0.25em;">
...
$sel1=executework("select * from tob_auction,tob_platform where tob_auction.platf=tob_platform.id and state='".$stat."' and tdate='".$adate."' and tob_platform.isactive=1 order by field(tob_platform.catg,'NBS','SBS','SLS','NLS'),tob_platform.seqid");
        ...
        <div class="col-lg-6">
           ...
           <div class="card">
		    ...
		    <h6 class="text-uppercase">CUMULATIVE INFORMATION (<?php echo strtoupper($stat1); ?>)</h6>
              ...
              <div class="card-body" style="padding: 0.25em;"> 
                ...
                <!-- Tab panes -->
                <div class="tab-content" style="padding: 0em;">
                  <div id="piil-3" class="container tab-pane active" style="padding: 0px;">
                      ...
                      <div> 
                          <div class="example-content">
							...
							<table border="0" style="width: 565px;">
                              <tr>
                                <td style="vertical-align: top"><div align="center"><!-- Example scripts go here -->
                            <div id="chart3" style="margin-top:20px; margin-left:20px; width:550px; height:500px;" align="center"></div></div></td>
          ...
          <div class="card">
            <!--<div class="card-header"><i class="fa fa-table"></i> Data Table Example</div>-->
            <?php
							$sel3=executework("select * from tob_grade where pdate<='".$qdate2."' and state='".$stat1."' order by pdate desc limit 1");
							$rowaa=@mysqli_fetch_array($sel3);
								$adate=$rowaa['pdate'];
			  ?>
              <div class="table-responsive"  style=" height: 500px; overflow: scroll;">
              <table id="" class="table table-bordered"  style="font-size: 12px;">
                ...
                <tbody>
 <?php
$sel1=executework("select * from tob_auction,tob_platform where tob_auction.platf=tob_platform.id and state='".$stat1."' and tdate='".$adate."' and tob_platform.isactive=1 order by field(tob_platform.catg,'NBS','SBS','SLS','NLS'),tob_platform.seqid");
							$cntt=@mysqli_num_rows($sel1);
							$qau=0;
							$tbal=0;
							$tquat=0;
                   ...
                   if($tquat>0)
							...
							$tcpr=0;
							 ?>
							  ...
							  <tr>
							    <td colspan="6" bgcolor="#FFFFFF" class="style36" style="text-align: center"><table width="80%" border="0" cellpadding="1" cellspacing="1" bgcolor="#000033" align="center" style="font-size: 12px;">
							<tr style="color:#008cff; font-size: 12px;" >
	...
	?>
         ...
         <div class="col-lg-3" style="padding-left: 10px; padding-right: 10px;">
           ...
           <a href="<?php echo $link1; ?>"><div class="card">
		  
		   
		    <h6 class="text-uppercase" style="background-color: #1e4c1b">VIEW <?php echo strtoupper($stat); ?></h6>
         ...
         <div class="col-lg-3" style="padding-left: 10px; padding-right: 10px;">
           ...
           <a href="<?php echo $link2; ?>"><div class="card">
		  
		   
		    <h6 class="text-uppercase" style="background-color: #1e4c1b">VIEW <?php echo strtoupper($stat1); ?></h6>
...
function graph3()
{
	var st1='<?php echo $sta ?>';
	var td1='<?php echo $tda ?>';
	var prc1='<?php echo $aprca ?>';
