...
<title>Employment Opportunities | Tobacco Board, Guntur</title>
...
<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  ?>	</td>
    ...
    <td colspan="2" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabbor">
      <tr>
        <td height="25" bgcolor="#F7F7F7"><div class="breadcrumb flat"> <a href="index.php">Home</a>
          <?php if(empty($_GET['ar'])) {?>
          <a href="#"> Employment Opportunities</a>
          <?php }else {?>
<a href="vacancies.php">Employment Opportunities</a> <a href="#"> Archives</a>
<?php }?></div></td>
        <td width="10%" bgcolor="#F7F7F7"><?php
		   		if(empty($_GET['prin']))
				{
		   ?>
          <a href="#" onclick="MM_openBrWindow('vacancies.php?ar=<?php echo $_GET['ar']?>&amp;prin=y','','width=800,height=600')"><img src="tob2_imgs/printButton.gif" border="0" /></a> <a href="#" onclick="MM_openBrWindow('vacancies.php?ar=<?php echo $_GET['ar']?>&amp;prin=y','','width=800,height=600')">Print</a>
          <?php
		   		}
		   ?></td>
      </tr>
      ...
      <tr>
		...
		$select=executework("select * from tob_vacancies $archive order by id desc");
...
if ($count > 0)
{
	...
	$select1= executework("select * from tob_vacancies $archive order by id desc LIMIT $pagenow1, $max_recs_per_page");
	...
	?>
	  ...
	  for($im=1;$im<=$pages;$im++)
	  {
			...
			{
				?>
                                          <a href="vacancies.php?page_index=<?php echo "$im" ?>&amp;ar=<?php echo $_GET['ar']?>" class="hlink1"><?php echo "$im" ?></a>&nbsp;
                                          <?php
			}
			else
			{
	...
	?>
			...
			{
				if($rows['tfile']!="")
				$link="../tbdata/vacanciesfiles/".$rows['tfile'];
				else
				$link="#";
		  ...
		  ?>
				<tr>
				  <td width="54%" align="justify" style="padding-left:15px;">
				  <?php
				  	if(!empty($rows['tfile']))
					{
				  ?>
				  &#8226;	<a href="<?php echo $link?>" <?php if($link!="#"){?>target="_blank"<?php }?>><?php echo $rows['hdescription']; if($fcheck[1]=='pdf'){?>&nbsp;<img src="tob2_imgs/pdf_icon.gif" width="12" height="15" border="0" /><?php }?></a>&nbsp;&nbsp;&nbsp;<a href="<?php echo $link?>" target="<?php if($link!="#"){?>_blank<?php }?>"><img src="tob2_imgs/newindow_icon.gif" border="0" />											
						</a>
				   <?php
				   	}
					else
  ...
  <tr>
    ...
    <td colspan="2" valign="top"><table width="100%" border="0">
      <tr>
        <td width="39%">&nbsp;
          <div align="right">Page Updated on : 
            <?php if(!empty($row['tdate'])){?>
            <span class="update"><?php echo datepattrn($row['tdate'])?></span>
            <?php }?>
          </div></td>
        <td width="41%"><div align="right"><span style="padding-left:15px;">
          <?php
				if(!empty($_GET['ar']) && $_GET['ar']==1)			
				{
			?>
          <a href="vacancies.php" class="style30">&laquo; Back To Vacancies</a>
          <?php
				}
				else
				{
			?>
          <a href="vacancies.php?ar=1" class="style30">&raquo; Archives</a>
          <?php	
				}
			?>
        </span></div></td>
  ...
  <tr>
    ...
    <td width="832" valign="top"><img src="tob2_imgs/spacer.png" width="535" height="1" /></td>
