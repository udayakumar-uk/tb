...
if($_FILES['image']['name']!="")
{
	...
	if($count>0)
	{
		...
		$update=executeupdate("update master_employee set image='".$f1."' where Employee_number='".$row['Employee_number']."'");
...
function GetDays($sStartDate, $sEndDate){  
  ...
  return $aDays;  
	...
	if($row['image']!="")
	$img="admin/employeeimage/".$row['image'];
	else
	$img="admin/noimage.png";
	...
	$selectn=executework("select * from master_employee where Employee_number<>''".$qr." order by Employee_number limit ".$n.",1");
	$countn=@mysqli_num_rows($selectn);
	$rown=@mysqli_fetch_array($selectn);
	
	//$selm=executework("select * from master_employee where 
?>
<a name="top" id="top"></a>
<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td width="74%" class="style1"><a href="employeedata.php?catg=<?php echo $catg ?>&group_by=<?php echo $grp ?>&empno=<?php echo $empn ?>&empname=<?php echo $empnm ?>">Back</a></td>
    <td width="8%" class="style8"><?php if($rowp['Employee_number']!="") { ?><a href="employee.php?catg=<?php echo $catg ?>&group_by=<?php echo $grp ?>&empno=<?php echo $empn ?>&eid=<?php echo $rowp['Employee_number'] ?>&n=<?php echo $n-1 ?>&empname=<?php echo $empnm ?>">&lt;&lt; Previous</a><?php } ?></td>
    <td width="18%" class="style8"><?php if($rown['Employee_number']!="") { ?><a href="employee.php?catg=<?php echo $catg ?>&group_by=<?php echo $grp ?>&empno=<?php echo $empn ?>&eid=<?php echo $rown['Employee_number'] ?>&n=<?php echo $n+1 ?>&empname=<?php echo $empnm ?>">Next &gt;&gt;</a><?php } ?></td>
  </tr>
</table>
</td>
						</tr>
  ...
  <div id="content">
      <div id="tab1">
        <table width="800" border="0" align="center" cellpadding="2" cellspacing="2" bgcolor="#FFFFFF">
                      <tr>
                        <td colspan="3" bordercolor="#000000" bgcolor="#FFFFFF"><table width="800" border="1" align="center" cellpadding="4" cellspacing="1" bordercolor="#FFFFFF" bgcolor="#000000">
                          ...
                          <tr bgcolor="#DDDDDD">
                            ...
                            <td bgcolor="#DDDDDD" class="style8"><?php echo $row['Designation']; ?></td>
                          ...
                          <tr bgcolor="#F5F5F5">
                            ...
                            <td class="style8"><?php echo $row['place_working']; ?></td>
                          ...
                          <tr bgcolor="#DDDDDD">
                            ...
                            <td bgcolor="#F5F5F5" class="style8">Rs. <?php echo $row['E_Pay_Band']; ?></td>
                            <td width="16%" bgcolor="#F5F5F5">&nbsp;</td>
                            <td width="18%" bgcolor="#F5F5F5">&nbsp;</td>
                            </tr>
                          <tr bgcolor="#DDDDDD">
                            <td class="style8a">Grade Pay</td>
                            <td class="style8">Rs. <?php echo $row['E_Grade_Pay']; ?></td>
                   ...
                   <?php
				   $selser=executework("select * from service_details where E_NO='".$row['Employee_number']."' order by E_FROM");
                  ...
                  <?php
				   ...
				   $selv=executework("select * from disciplinary_det where E_NO='".$row['Employee_number']."' order by id");
...
<table width="800" border="0" align="center" cellpadding="2" cellspacing="2" bgcolor="#FFFFFF">
                   ...
                   <?php
				   $selpr=executework("select * from promotion_det where E_NO='".$row['Employee_number']."'");
				 ...
				 ?>
                   <?php
				   $selpr=executework("select * from tob_aaleaves where empno='".$row['Employee_number']."' order by edate");
				   $cntp=@mysqli_num_rows($selpr);
				   if($cntp>0)
				   {
				   		//$rowp=@mysqli_fetch_array($selpr);
