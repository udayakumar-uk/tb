...
<title>Employee Corner | Tobacco Board, Guntur</title>
...
<table width="99%" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
  	<?php
		...
		{
	?>
<td width="223" rowspan="2" valign="top" bgcolor="#ededed" >
    ...
    <td colspan="2" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="tabbor">
      <tr>
        <td width="90%" height="25" bgcolor="#F7F7F7"><div class="breadcrumb flat"> <a href="index.php">Home</a>
          ...
          <?php }else {?>
<a href="empcorner.php?stype=<?php echo $_GET['stype'];?>">Employee Corner</a> <a href="#">Archives</a>
<?php }?><a href="#"><?php echo $stp ?></a></div></td>
        <td width="10%" bgcolor="#F7F7F7"><?php
		   		if(empty($_GET['prin']))
				{
		   ?>
          <a href="#" onclick="MM_openBrWindow('empcorner.php?stype=<?php echo $_GET['stype']?>&ar=<?php echo $_GET[ar]?>&prin=y','','width=800,height=600')"><img src="tob2_imgs/printButton.gif" border="0" /> </a><a href="#" onclick="MM_openBrWindow('empcorner.php?stype=<?php echo $_GET['stype']?>&ar=<?php echo $_GET['ar']?>&prin=y','','width=800,height=600')">Print</a>
          <?php
		   		}
		   ?></td>
      </tr>
...
?>						
	...
	?>
	  ...
	  for($im=1;$im<=$pages;$im++)
	  {
			...
			{
				?>
                                          <a href="empcorner.php?page_index=<?php echo "$im" ?>&amp;stype=<?php echo $_GET['stype']?>&amp;ar=<?php echo $_GET['ar']?>" class="hlink1"><?php echo "$im" ?></a>&nbsp;
                                          <?php
			}
			else
			{
	...
	?>
			...
			{
				if($rows['tfile']!="")
				$link="../tbdata/employeefiles/".$rows['tfile'];
				else
				$link="#";
				$fcheck=explode(".",$rows['tfile']);
				if(!empty($rows['tfile']))
				{
					if(!empty($rows['horder_details']))
					$ord=$rows['horder_details'];
					else
					$ord=$rows['order_details'];
		  ?>
					  <li>
						<a href="<?php echo $link?>" target="<?php if($link!="#"){?>_blank<?php }?>"><?php echo $ord; if($fcheck[1]=='pdf'){?>&nbsp;<img src="tob2_imgs/pdf_icon.gif" width="12" height="15" border="0" /><?php }?></a>&nbsp;&nbsp;&nbsp;<a href="<?php echo $link?>" target="<?php if($link!="#"){?>_blank<?php }?>"><img src="tob2_imgs/newindow_icon.gif" border="0" />											
						</a><br />
					  </li>
             <?php	
			 	}
  ...
  <tr>
    ...
    <td colspan="2" valign="top"><table width="100%" border="0">
      <tr>
        <td width="39%">&nbsp;</td>
        <td width="41%"><div align="center"><span style="padding-left:15px;">
          <?php
				if(!empty($_GET['ar']) && $_GET['ar']==1)			
				{
			?>
          <a href="empcorner.php?stype=<?php echo $_GET['stype']?>" class="style30">&laquo; Back To Employee Corner </a>
          <?php
				}
				else
				{
			?>
          <a href="empcorner.php?stype=<?php echo $_GET['stype']?>&amp;ar=1" class="style30">&raquo; Archives</a>
          <?php	
				}
			?>
        </span></div></td>
  ...
  <tr>
    ...
    <td width="833" valign="top"><img src="tob2_imgs/spacer.png" width="535" height="1" /></td>
